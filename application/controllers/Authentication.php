...
class Authentication extends CRM_Controller
{
    ...
    public function admin()
    {
        ...
        $this->form_validation->set_rules('email', _l('admin_auth_login_email'), 'trim|required');
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                $staffbranch = $this->input->post('staffbranch');
                $year_id = $this->input->post('year_id');
                $data = $this->Authentication_model->login($email, $password, $remember, true,$staffbranch,$year_id);
        ...
        $data['year_info'] = $this->db->query("SELECT * from tblfinancialyear where status = 1 order by slug desc ")->result();
	...
	public function getstaffbranch() {
	   ...
	   /* $staffbranchdata= $this->db->query("SELECT sc.*,s.email FROM `tblstaff` s  LEFT JOIn `tblcompanybranchperson` cbp ON cbp.staffid=s.`staffid` LEFT JOIN `tblcompanybranch` sc ON cbp.`comp_branch_id`=sc.`id` WHERE s.`email`='".$email."' AND sc.id!='NULL' GROUP BY `id`")->result_array();
		echo json_encode($staffbranchdata);*/

        $staff_info= $this->db->query("SELECT * from tblstaff where `email` = '".$email."' || `user_id` = '".$email."' ")->row();
        $html = $fhtml = "";
        if(!empty($staff_info)){
            $staffbranchdata= $this->db->query("SELECT * from tblcompanybranch where id IN (".$staff_info->branch_id.")  ")->result();

            if(!empty($staffbranchdata)){
                if (count($staffbranchdata) >= 1){
                    $html = '<select class="form-control selectpicker" required="" id="staffbranch" name="staffbranch" data-live-search="true"><option value=""> Select Company Branch</option>';
                    foreach ($staffbranchdata as $key => $value) {
                        $b_name = $value->comp_branch_name.' - '.$value->email_id;
                         $html .= '<option value="'.$value->id.'">'.$b_name.'</option>';
                    }
                    $html .= '</select>';
                }else{
                    $html = '<input type="hidden" name="staffbranch" value="'.$staff_info->branch_id.'">';
                }
            }
            
            if ($staff_info->canBackDateEntry == 1){
                $year_info = $this->db->query("SELECT * from tblfinancialyear where status = 1 order by slug desc ")->result();
                $fhtml = '<select class="form-control selectpicker" required="" id="year_id" name="year_id" data-live-search="true"><option value=""> Select Financial Year</option>';
                if (!empty($year_info)) {
                    foreach ($year_info as $key => $value) {
                        $fhtml .= '<option value="' . $value->id . '">' . $value->name . '</option>';
                    }
                }
                $fhtml .= '</select>';
            }else{
                
                $fhtml = '<input type="hidden" name="year_id" value="'.getCurrentFinancialYear().'">';
            }
            
        }
        
        echo json_encode(array("staff_branch"=> $html, "financial_year"=> $fhtml));
