...
class Requests_API extends Clients_controller
{
			...
			$request_list = $this->db->query("SELECT * FROM `tblrequests` where category = '".$category."' and cancel = 0 and  (addedfrom = '".$user_id."' || addedby = '".$user_id."')  ORDER by id desc")->result();
			   ...
			   if(!empty($request_list)){
				   ...
				   foreach($request_list as $row){
					...
					$get_notification_user_info=$this->home_model->get_result('tblnotifications', array('table_id'=>$row->id,'module_id'=>1,'isread'=>1),  array('id','readdate','touserid'),'');
    ...
    { 
	  ...
	  $return_arr = array();
		...
		//http://bookallservices.com/schach/Requests_API/approve_request?user_id=1&approve_status=1&remark=Approved&approved_amount=500&request_id=1
			...
			$check = $this->home_model->get_row('tblrequests', array('confirmed_by_user'=>0,'cancel'=>0,'approved_status'=>0,'addedfrom'=>$user_id), '');
				...
				if(!empty($check)){
					...
					if($category != 4){
						...
						$check = $this->home_model->get_row('tblrequests', array('confirmed_by_user'=>0,'cancel'=>0,'approved_status'=>1,'addedfrom'=>$user_id), '');
