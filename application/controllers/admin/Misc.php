...
class Misc extends Admin_controller
{
    ...
    public function add_vendor_note($rel_id, $rel_type)
    {
        if ($this->input->post()) {
            $success = $this->misc_model->add_vendor_note($this->input->post(), $rel_type, $rel_id);
            if ($success) {
                set_alert('success', _l('added_successfully', _l('vendor_note')));
            }
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
	...
	public function edit_vendor_note($id)
    {
        if ($this->input->post()) {
            $success = $this->misc_model->edit_vendor_note($this->input->post(), $id);
            echo json_encode([
                'success' => $success,
                'message' => _l('note_updated_successfully'),
            ]);
	...
	public function delete_vendor_note($id)
    {
        $success = $this->misc_model->delete_vendor_note($id);

        if (!$this->input->is_ajax_request()) {
            if ($success) {
                set_alert('success', _l('deleted', _l('vendor_note')));
            }
            redirect($_SERVER['HTTP_REFERER']);
        } else {
            echo json_encode(['success' => $success]);
