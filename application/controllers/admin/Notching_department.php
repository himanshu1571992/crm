...
class Notching_department extends Admin_controller {
    ...
    public function index() {
        ...
        check_permission(182,'view');    

        $where = " from_department = '3' ";
    	...
    	$data['title'] = 'Notching Item Request List';
        $this->load->view('admin/notching_department/view_request', $data);
    }
   

    public function add_request($id = '') {
        check_permission(182,'create'); 
        if ($this->input->post()) {
            extract($this->input->post());
            /*echo '<pre/>';
            print_r($_POST);
            ...
            if(!empty($item_id)){
                ...
                if($this->home_model->insert('tbldepartmentrequest', $ad_data)){
                    ...
                    redirect(admin_url('notching_department')); 
                }
            }else{
            	set_alert('danger', 'Components are empty!');
                redirect(admin_url('notching_department/add_request'));
            }            
        }
        ...
        $data['title'] = 'Notching Item Request';

        $superior_ids = $this->db->query("SELECT superior_ids from tblproduction_department where id = '3' ")->row()->superior_ids;
        ...
        $this->load->view('admin/notching_department/add_request', $data);
    }
    ...
    public function get_component_details() {
        ...
        $superior_ids = value_by_id('tblproduction_department',3,'superior_ids');
            ...
            <div class="panel_s">
                <div class="panel-body">
                    ...
                    <div class="btn-bottom-toolbar text-right">
                            <button class="btn btn-info" type="button" id="submit_request">Send Request</button>
                        </div>
                </div>
    ...
    public function get_confirmation_html()
    {
        if(!empty($_POST)){
            ...
            ?>
            <form action="<?php echo admin_url('notching_department/request_confirmation');?>" class="proposal-form" enctype="multipart/form-data" method="post" accept-charset="utf-8">
            <div class="row">
    ...
    public function conversion_list($id = '') {
        ...
        $this->load->view('admin/notching_department/conversion_list', $data);

    }


    public function conversion($id = '') {
        check_permission(184,'create'); 
        if ($this->input->post()) {
            extract($this->input->post());
            /*echo '<pre/>';
            print_r($_POST);
            ...
            if(!empty($item_id)){
                ...
                if($this->home_model->insert('tbldepartmentconversion', $ad_data)){
                    ...
                    if($waste > 0){
                        $waste_info = $this->db->query("SELECT * from tblprostock where warehouse_id = '".$warehouse_id."' and department_id = '3' and pro_id = '0' and status = 1 and stock_type = 5  ")->row();
                    ...
                    $stock_info = $this->db->query("SELECT * from tblprostock where warehouse_id = '".$warehouse_id."' and service_type = '".$service_type."' and department_id = '3' and pro_id = '".$product_id."' and status = 1 and stock_type = 1  ")->row();
                    ...
                    foreach($item_id as $id) {
                        ...
                        //start Less stock
                        $stock_info = $this->db->query("SELECT * from tblprostock where warehouse_id = '".$warehouse_id."' and service_type = '".$service_type."' and department_id = '3' and pro_id = '".$id."'   ")->row();
                    ...
                    redirect(admin_url('notching_department/conversion_list')); 
                }
            }else{
                set_alert('danger', 'Components are empty!');
                redirect(admin_url('notching_department/conversion'));
            }            
        }
        ...
        $this->load->view('admin/notching_department/conversion', $data);
    }
                                ...
                                <div class="form-group" >
                                    <table class="table credite-note-items-table items table-main-credit-note-edit no-mtop saletable">
                                        ...
                                        if(!empty($item_info)){
                                            $i = 1;
                                            foreach ($item_info as $value) {
                                                $stock_info = $this->db->query("SELECT qty from tblprostock where warehouse_id = '".$warehouse_id."' and service_type = '".$service_type."' and department_id = '3' and pro_id = '".$value->item_id."' and status = 1 and stock_type = 1  ")->row();
                                                $available_qty = 0;
                                                if(!empty($stock_info)){
    ...
    public function demand() {
        ...
        $this->load->view('admin/notching_department/demand', $data);
    }
    ...
    public function get_details()
    {
        if(!empty($_POST)){
            ...
            <form action="<?php echo admin_url('notching_department/approve_demand');?>" class="proposal-form" enctype="multipart/form-data" method="post" accept-charset="utf-8">
            <div class="row">
    ...
    public function approve_demand() {
        if(!empty($_POST)){
            ...
            if($this->home_model->update('tbldepartmentrequestitems', $ad_data,array('id'=>$id))){
                ...
                redirect(admin_url('notching_department/demand')); 
            }
    ...
    public function store() {
        ...
        $data['title'] = 'Notching Store';
        $this->load->view('admin/notching_department/store', $data);
    }
