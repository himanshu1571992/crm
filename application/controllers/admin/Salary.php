...
class Salary extends Admin_controller
{
	...
	public function advance_salary_report($id='')
    {
		...
		if(!empty($_POST)){
			...
			$where = " category = 1 and approved_status = 1 and confirmed_by_user = 1 and receive_status=1 and cancel = 0 and date between '".$f_date."' and '".$t_date."' ";
			if(!empty($staff_id)){
				$data['s_staff'] = $staff_id;
				$where .= "and addedfrom = '".$staff_id."'";
			}

			$data['advance_salary_info'] = $this->db->query("SELECT * FROM `tblrequests` where ".$where."  ")->result();
    ...
    public function employee_ctc($id = '', $section="") {
        check_permission(110, 'view');
        $data['title'] = 'Employee CTC Calculator';
        if(!empty($section) && !empty($id) && $section == "SI"){
            $data["name"] = value_by_id_empty(" tblstaffinterviewdetails", $id, "candidate_name");
    ...
    public function ctc_print($id='')
    {
		if(!empty($_POST)){
			...
			$other_allowance = get_new_other_allowance($salary,$type);
			$data['ctc_info'] = get_new_ctc($salary,$other_allowance,$type);
			if(!empty($name)){
				$data['name'] = $name;
			}else{
				$data['name'] = '--';
	...
	public function download_ctc() {
        if (!empty($_POST)) {
            extract($this->input->post());
            
            /* $other_allowance = final_other_allowance($salary);
              $ctc_info = get_ctc($salary,$other_allowance); */

            $other_allowance = get_new_other_allowance($salary, $type);
            $ctc_info = get_new_ctc($salary, $other_allowance, $type);

            $file_name = 'Employee_CTC';

            $html = ctc_pdf($ctc_info, $name, $type);
            /* echo $html;
              die; */
            $dompdf = new Dompdf();
...
//            $dompdf->stream($file_name);
            ...
            if (isset($candidate_id) && !empty($candidate_id)) {
                ...
                file_put_contents(STAFF_ASSIGN_CTC_FOLDER.  $filename, $dompdf->output()); 
                $this->home_model->update("tblstaffinterviewdetails", array("generate_ctc" => $filename), array("id" => $candidate_id));
            }
            $dompdf->stream($file_name);
        }
    }

    public function export_afterpay($year,$month)
    { 
        if(!empty($year) && !empty($month)){
			
			$j_date = $year.'-'.$month.'-31';
			
			$staff_list = $this->db->query("SELECT * FROM `tblstaff` where active = 1 and taxable = 1 and joining_date <= '".$j_date."' ")->result();
			...
			if(!empty($staff_list)){
				foreach($staff_list as $staff_row){
					...
					$delete_last = $this->home_model->delete('tblsalaryafterpaidlog', array('staff_id'=>$staff_id,'month'=>$month,'year'=>$year,'taxable'=>1));	
					...
					$ad_data = array(
							...
							'arrear_amt' => 0,
							'tds_amt' => 0,
							'lockDownDeduction_amt' => 0,
							'create_at' => $paid_date,
							'status' => 1,
							'taxable' => 1
						);
					
					$insert = $this->home_model->insert('tblsalaryafterpaidlog', $ad_data);
				...
				if($insert){
					...
					$salary_info = $this->db->query("SELECT sp.*, e.gender,e.payment_mode,e.firstname,e.account_no,e.monthly_salary FROM `tblsalaryafterpaidlog` as sp LEFT JOIN tblstaff as e ON sp.staff_id = e.staffid  WHERE sp.month='".$month."' and sp.year='".$year."' and sp.status='1' and sp.taxable='1' ")->result();	
					...
					$objPHPExcel->getActiveSheet()->SetCellValue('E4', 'Gross Salary fixed');       
					$objPHPExcel->getActiveSheet()->SetCellValue('F4', 'No of days Worked');       
					$objPHPExcel->getActiveSheet()->SetCellValue('G4', 'Total Paid Leave');       
					$objPHPExcel->getActiveSheet()->SetCellValue('H4', 'Basic');       
					$objPHPExcel->getActiveSheet()->SetCellValue('I4', 'HRA');       
					$objPHPExcel->getActiveSheet()->SetCellValue('J4', 'Conv. Allowance');       
					$objPHPExcel->getActiveSheet()->SetCellValue('K4', 'Medical Allowance');       
					$objPHPExcel->getActiveSheet()->SetCellValue('L4', 'Uniform Allowance');       
					$objPHPExcel->getActiveSheet()->SetCellValue('M4', 'Other Allowance');       
					$objPHPExcel->getActiveSheet()->SetCellValue('N4', 'Gross Salary Earned');       
					$objPHPExcel->getActiveSheet()->SetCellValue('O4', 'PF Deduction');       
					$objPHPExcel->getActiveSheet()->SetCellValue('P4', 'ESIC Deduction');       
					$objPHPExcel->getActiveSheet()->SetCellValue('Q4', 'PT Deduction');       
					$objPHPExcel->getActiveSheet()->SetCellValue('R4', 'TDS');       
					$objPHPExcel->getActiveSheet()->SetCellValue('S4', 'Loan');       
					$objPHPExcel->getActiveSheet()->SetCellValue('T4', 'Advance Salary');       
					$objPHPExcel->getActiveSheet()->SetCellValue('U4', 'Conv Deduction');       
					$objPHPExcel->getActiveSheet()->SetCellValue('V4', 'Net Salary');       
					// set Row
					$rowCount = 5;
					$i = 1;
					foreach ($salary_info as $val) 
					{
					   ...
					   if($val->payment_mode == 3 || $val->payment_mode == 0){
						...
						$objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $val->monthly_salary);
						$objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $val->present_days);
						$objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $val->paid_leave);
						$objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $val->bacis_salary);
						$objPHPExcel->getActiveSheet()->SetCellValue('I' . $rowCount, $val->hra_amt);
						$objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, $val->ta_amt);
						$objPHPExcel->getActiveSheet()->SetCellValue('K' . $rowCount, $val->medical_amt);
						$objPHPExcel->getActiveSheet()->SetCellValue('L' . $rowCount, $val->uniform_amt);
						$objPHPExcel->getActiveSheet()->SetCellValue('M' . $rowCount, $val->other_amt);
						$objPHPExcel->getActiveSheet()->SetCellValue('N' . $rowCount, $val->gross_salary);
						$objPHPExcel->getActiveSheet()->SetCellValue('O' . $rowCount, $val->pf_amt);
						$objPHPExcel->getActiveSheet()->SetCellValue('P' . $rowCount, $val->esic_amt);
						$objPHPExcel->getActiveSheet()->SetCellValue('Q' . $rowCount, $val->pt_amt);
						$objPHPExcel->getActiveSheet()->SetCellValue('R' . $rowCount, $val->tds_amt);
						$objPHPExcel->getActiveSheet()->SetCellValue('S' . $rowCount, $val->loan);
						$objPHPExcel->getActiveSheet()->SetCellValue('T' . $rowCount, $val->advance);
						$objPHPExcel->getActiveSheet()->SetCellValue('U' . $rowCount, $val->expense);
						$objPHPExcel->getActiveSheet()->SetCellValue('V' . $rowCount, $val->net_salary);
	...
	public function export_afterpaycash($year,$month)
    { 
        if(!empty($year) && !empty($month)){
			
			$j_date = $year.'-'.$month.'-31';
			
			$staff_list = $this->db->query("SELECT * FROM `tblstaff` where active = 1 and taxable = 2 and joining_date <= '".$j_date."' ")->result();
			...
			if(!empty($staff_list)){
				foreach($staff_list as $staff_row){
					...
					$delete_last = $this->home_model->delete('tblsalaryafterpaidlog', array('staff_id'=>$staff_id,'month'=>$month,'year'=>$year,'taxable'=>2));	
				...
				if($insert){
					...
					$salary_info = $this->db->query("SELECT sp.*, e.gender,e.payment_mode,e.firstname,e.account_no,e.monthly_salary FROM `tblsalaryafterpaidlog` as sp LEFT JOIN tblstaff as e ON sp.staff_id = e.staffid  WHERE sp.month='".$month."' and sp.year='".$year."' and sp.status='1' and sp.taxable='2' ")->result();	
