...
class ClientBranch extends Admin_controller {
    ...
    public function branch($id = '') {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('client_branch')));
                    if ($save_and_add_contact == false) {
                        //redirect(admin_url('ClientBranch/branch/' . $id));
                        redirect(admin_url('ClientBranch/'));
                    } else {
                        //redirect(admin_url('ClientBranch/branch/' . $id . '?group=contacts&new_contact=true'));
                        redirect(admin_url('ClientBranch/'));
                    }
                }
            } else {
                check_permission(80,'edit');
                ...
                if ($success == true) {
                    set_alert('success', _l('updated_successfully', _l('client_branch')));
                }
               // redirect(admin_url('ClientBranch/branch/' . $id));
                redirect(admin_url('ClientBranch/'));
            }
        ...
        if ($group != 'contacts' && $contact_id = $this->input->get('contactid')) {
            redirect(admin_url('ClientBranch/branch/' . $id . '?group=contacts&contactid=' . $contact_id));
        }
        ...
        if ($id == '') {
            ...
            $data['client'] = $client;
            $title = $client->client_branch_name;
    ...
    public function save_longitude_and_latitude($client_id) {
        ...
        $this->db->update('tblclientbranch', [
    ...
    public function assign_admins($id) {
        ...
        redirect(admin_url('ClientBranch/branch/' . $id . '?tab=customer_admins'));
    }
    ...
    public function delete_customer_admin($customer_id, $staff_id) {
        ...
        redirect(admin_url('ClientBranch/branch/' . $customer_id) . '?tab=customer_admins');
    }
    ...
    public function delete_contact($customer_id, $id) {
        ...
        redirect(admin_url('ClientBranch/branch/' . $customer_id . '?group=contacts'));
    }
    ...
    public function delete($id) {
        ...
        if (is_array($response) && isset($response['referenced'])) {
            ...
            set_alert('success', _l('deleted', _l('client_branch')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('client_branch_lowercase')));
        }
        redirect(admin_url('ClientBranch'));
    }
    ...
    public function zip_credit_notes($id) {
        ...
        if ($this->input->post()) {
            ...
            if (count($credit_notes) == 0) {
                ...
                redirect(admin_url('ClientBranch/branch/' . $id . '?group=credit_notes'));
            }
    ...
    public function zip_invoices($id) {
        ...
        if ($this->input->post()) {
            ...
            if (count($invoices) == 0) {
                ...
                redirect(admin_url('ClientBranch/branch/' . $id . '?group=invoices'));
            }
            mkdir($dir, 0777);
    ...
    public function zip_estimates($id) {
        ...
        if ($this->input->post()) {
            ...
            if (count($estimates) == 0) {
                ...
                redirect(admin_url('ClientBranch/branch/' . $id . '?group=estimates'));
            }
            mkdir($dir, 0777);
    ...
    public function zip_payments($id) {
        ...
        $this->db->where('tblclientbranch.userid', $id);
        if (!$has_permission_view) {
            $whereUser = '';
        ...
        $this->db->join('tblclientbranch', 'tblclientbranch.userid = tblinvoices.clientid', 'left');
        ...
        if (count($payments) == 0) {
            ...
            redirect(admin_url('ClientBranch/branch/' . $id . '?group=payments'));
        }
        mkdir($dir, 0777);
            ...
            if (isset($_FILES['file_csv']['name']) && $_FILES['file_csv']['name'] != '') {
                ...
                if (!empty($tmpFilePath) && $tmpFilePath != '') {
                    ...
                    if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                        ...
                        $db_temp_fields = $this->db->list_fields('tblclientbranch');
                            ...
                            if (count($insert) > 0) {
                                ...
                                if (!$this->input->post('simulate')) {
                                    ...
                                    if (isset($insert['company']) && $insert['company'] != '' && $insert['company'] != '/') {
                                        if (total_rows('tblclientbranch', ['company' => $insert['company']]) === 1) {
                                            $this->db->where('company', $insert['company']);
                                            $existingCompany = $this->db->get('tblclientbranch')->row();
                                            $tmpInsert = [];
    ...
    public function statement_pdf() {
        ...
        /*if (!has_permission('invoices', '', 'view') && !has_permission('payments', '', 'view')) {
            set_alert('danger', _l('access_denied'));
            redirect(admin_url('ClientBranch/branch/' . $customer_id));
        }*/
    ...
    public function send_statement() {
        ...
        redirect(admin_url('ClientBranch/branch/' . $customer_id . '?group=statement'));
    }
    ...
    {
        check_permission('86,250','view');
        $data['status_info']  = $this->db->query("SELECT * FROM tblclientstatus WHERE status =  '1' ")->result();

        $data['title'] = 'Client Status';
        $this->load->view('admin/clients/client_status', $data);
    }
    ...
    public function add_client_status($id="")
    {
        ...
        if(!empty($_POST)){
            ...
            if($insert){
                ...
                redirect(admin_url('ClientBranch/client_status'));
            }
        }

        if(!empty($id)){
            $data['status_info']  = $this->db->query("SELECT * FROM tblclientstatus WHERE id = '".$id."'")->row();
            $data['id'] = $id;
            $data['title'] = 'Edit Client Status';
        }else{
            $data['title'] = 'Add Client Status';
        }
    ...
    public function edit_client_status($id="")
    {

        check_permission('86,250','edit');
        ...
        if(!empty($_POST)){
            ...
            $update = $this->home_model->update('tblclientstatus', $ad_data,array('id'=>$id));

            if($update){

                set_alert('success', 'Client status updated Successfully');
                redirect(admin_url('ClientBranch/client_status'));
            }
        }
    ...
    {
        check_permission('86,250','delete');

        $paymentfollowup = $this->db->query("SELECT * FROM `tblpaymentfollowupclients` Where status = '".$id."' ")->row();
        $clientbranch = $this->db->query("SELECT * FROM `tblclientbranch` Where client_status = '".$id."' ")->row();
        if (!empty($paymentfollowup) OR !empty($clientbranch)){
            set_alert('warning', "Can't be delete, its using anywhere.");
            redirect($_SERVER['HTTP_REFERER']); die;
        }

        $response = $this->home_model->delete('tblclientstatus', array('id'=>$id));
        if ($response == true) {
            set_alert('success', _l('deleted', 'Client Status'));
        } else {
            set_alert('warning', _l('problem_deleting', 'Client Status'));
        }
        redirect(admin_url('ClientBranch/client_status'));
    }

    public function assignsalesperson($clientbranch_id){
        if(!empty($_POST)){
            extract($this->input->post());

            $salesperson = implode(',', $sales_parson);
            $response = $this->home_model->update('tblclientbranch', array('sales_parson' => $salesperson), array('userid' => $clientbranch_id));
            if ($response){
                set_alert('success', 'Sales person assign successfully');
            }else{
                set_alert('warning', _l('problem_deleting', 'Client Status'));
            }
            redirect(admin_url('ClientBranch'));
        }
        $sales_person = client_info($clientbranch_id)->sales_parson;
        $sales_person_info = $this->db->query("SELECT `sales_person_id` from `tblleadstaffgroup` where status = 1  ")->result();
    ?>
        <div class="row">
            <div class="col-md-12">
            <?php echo form_open($this->uri->uri_string(), array('id' => 'assignperosn', 'class' => 'assignperosn-form')); ?>
                <div class="form-group">
                    <label for="assign_sales_parson" class="control-label"> Assign Sales Parson</label>
                    <select class="form-control selectpicker" required="" data-live-search="true" multiple="" id="sales_parson" name="sales_parson[]">
                        <option value=""></option>
                        <?php
                        if (isset($sales_person_info) && count($sales_person_info) > 0) {
                            foreach ($sales_person_info as $sales_person_value) {
                                $sparsons = explode(',', $sales_person);
                                ?>
                                <option value="<?php echo $sales_person_value->sales_person_id ?>" <?php echo (in_array($sales_person_value->sales_person_id, $sparsons)) ? 'selected=""':''; ?>><?php echo cc(get_employee_name($sales_person_value->sales_person_id)); ?></option>
                                <?php
                            }
                        }
                        ?>
                ...
                <input type="hidden" name="clientbranch_id" value="<?php echo $clientbranch_id; ?>">
