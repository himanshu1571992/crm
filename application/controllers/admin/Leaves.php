   ...
   public function leave($id = '')
    {
		...
		$Staffgroup =  get_staff_group(15,get_staff_user_id());
		$i=0;
		$stafff=array();
		if(!empty($Staffgroup)){
    ...
    public function categories()
    {
	 ...
	 public function add_category($id = '') {
	 	check_permission('130,262','create');
        if ($this->input->post()) {
    ...
    {
        check_permission('130,262','delete');
        if (!$id) {
            redirect(admin_url('leaves/categories'));
        }
	...
	public function leave_approval_old($id = '')
    {
		if(!empty($id)){
			$data['request'] = $this->home_model->get_row('tblleaves', array('id'=>$id), '');
			...
			if(!empty($data['request']) && $data['request']->approved_status == 0){
				...
				if($this->input->post()) {
					...
					$description = ($approve_status==1) ? 'Leave Approve Successfully' :'Leave Decline';
					$notified = add_notification([
						'description'     => $description,
						'touserid'        => $creatorid,
						'fromuserid'      => get_staff_user_id(),
		...
		if(!empty($id)){
			...
			// if(!empty($data['request']) && $data['request']->approved_status == 0){
				...
				if($this->input->post()) {
					...
					/* This is code use for send request for take action of leave */
					$post_fields = array(
					...
					/* SEND REQUEST FOR LEAVE APPROVE START */
						$request_url = base_url()."Leaves_API/approve_leave";
					...
					/* SEND REQUEST FOR LEAVE APPROVE END */
				}

				$data["request_info"] = $data["read_by_user"] = array();
				$request_url = base_url()."Leaves_API/get_single_leave?leave_id=".$id;
				// $request_url = "https://schachengineers.com/schacrm/Leaves_API/get_single_leave?leave_id=754";
				$response = file_get_contents($request_url);
				if (!empty($response)){
					$decode_response = json_decode($response, TRUE);
					if (isset($decode_response["leave_info"]) && !empty($decode_response["leave_info"])){
						$data["request_info"] = $decode_response["leave_info"];
				...
				$data['title']      = 'Leave Request Approval';
				$this->load->view('admin/leaves/leave_approval', $data);
			// }else{
			// 	set_alert('warning', 'Action Already Taken On Request');
			// 	redirect(admin_url('leaves/list_leaves'));
	...
	public function setting_table() {
	 ...
	 public function add_setting($id = '') {
       	check_permission('131,263','create');

		$data['leave_categories'] = $this->home_model->get_result('tblleavescategories', array('status'=>1), '', array('name', 'asc'));
	...
	public function delete_leaves_setting($id) {
		check_permission('131,263','delete');
            $unit_data = array(
                'id' => $id
            );
	...
	public function leave_report()
    {
        ...
        if(!empty($_POST)){
        	...
        	if(!empty($category_id)){
        		...
        		$data['s_category_id'] = $category_id;
        	}
        	...
        	if(!empty($status)){
        		...
        		$where .= " and approved_status = '".$l_status."' ";
        	...
        	if(!empty($from_date) && !empty($to_date)){
				...
				$where .= " and from_date BETWEEN '".$from_date."' and '".$to_date."' ";
        	}

        	$data['leave_info'] = $this->db->query("SELECT * FROM `tblleaves` where $where   ")->result();


        }else{
        	$data['leave_info'] = $this->db->query("SELECT * FROM `tblleaves` where month(from_date) = '".date('m')."' and YEAR(from_date) = '".date('Y')."'   ")->result();
        }


        $data['title'] = 'Employee Leave Report';
        $this->load->view('admin/leaves/leave_report', $data);
