...
class Clients extends Admin_controller {
    ...
    public function client($id = '') {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                if ($success == true) {
                    set_alert('success', _l('updated_successfully', _l('ClientBranch')));
                }
                redirect(admin_url('clients/client/' . $id));
            }
        }
        ...
        if ($id == '') {
            ...
            if ($group == 'profile') {
                ...
                $this->load->model('projects_model');
                $data['project_statuses'] = $this->projects_model->get_project_statuses();
            } elseif ($group == 'client_ledger_reco') {
                $data['ledger_reco_data'] = $this->db->query("SELECT * FROM `tblclient_vendor_givenledger` WHERE `status`= '1' AND `client_id`=".$id." ORDER BY id DESC ")->result();
            } elseif ($group == 'add_client_ledger_reco') {
        ...
        $data['document_info']  = $this->db->query("SELECT * FROM tblclientdocuments WHERE parent_id = '".$parent_id."' and client_id = '".$id."' and status = '1'  order by type ASC")->result();
    ...
    public function add_client_ledger_reco(){
        if(!empty($_POST)){
            ...
            if (isset($edit_id)){
                ...
                set_alert('success', 'Client Ledger Reco added successfully');
            }else{
                $insert_id = $this->home_model->insert('tblclient_vendor_givenledger', $insertdata);
                if ($insert_id){
                    upload_ledger_reco($insert_id);
                    set_alert('success', 'Client Ledger Reco updated successfully');
                }
            }
            redirect(admin_url('clients/client/'.$client_id.'?group=client_ledger_reco'));    
        }
    }

    public function delete_ledger_reco($id) {
        $this->load->model('home_model');
        $getoldfile = value_by_id_empty('tblclient_vendor_givenledger', $id, 'file');
        $client_id = value_by_id_empty('tblclient_vendor_givenledger', $id, 'client_id');
    ...
    public function get_editdata()
    {
         ...
         if(!empty($_POST)){
            ...
            $folder_info  = $this->db->query("SELECT * FROM tblclientdocuments WHERE id =  '".$id."'  ")->row();
    ...
    public function invoice_search() {
        if(!empty($_POST)){
           ...
           redirect(admin_url('clients/client/'.$clientid.'?group=invoices&from_date='.$date_range['from_date'].'&to_date='.$date_range['to_date'].'&range='.$range.'&f_date='.$f_date.'&t_date='.$t_date));
        }
    }
    public function quotation_search() {
        if(!empty($_POST)){
           ...
           $date_range = get_date_range($range); 
           redirect(admin_url('clients/client/'.$clientid.'?group=proposals&from_date='.$date_range['from_date'].'&to_date='.$date_range['to_date'].'&range='.$range.'&f_date='.$f_date.'&t_date='.$t_date));
        }
    }

    public function estimate_search() {
        if(!empty($_POST)){
            extract($this->input->post());

           $date_range = get_date_range($range); 
           redirect(admin_url('clients/client/'.$clientid.'?group=estimates&from_date='.$date_range['from_date'].'&to_date='.$date_range['to_date'].'&range='.$range.'&f_date='.$f_date.'&t_date='.$t_date));
        }
    }
    ...
    public function payments_search() {
        if(!empty($_POST)){
           ...
           $date_range = get_date_range($range); 
           redirect(admin_url('clients/client/'.$clientid.'?group=payments&from_date='.$date_range['from_date'].'&to_date='.$date_range['to_date'].'&range='.$range.'&f_date='.$f_date.'&t_date='.$t_date));
        }
    }
    ...
    public function create_folder()
    {
        ...
        if(!empty($_POST)){
            $client_document = $this->input->post();

            $clientid = $client_document['client_id'];
            $parent_id = $client_document['parent_id'];
            $folder_name = $client_document['folder_name'];
                ...
                $this->load->model('home_model');    
                $insert = $this->home_model->insert('tblclientdocuments', $ad_data); 
           
          set_alert('success', 'Folder Created Successfully');
            

            if($parent_id > 0){
                redirect(admin_url('clients/client/'.$clientid.'?group=client_document&parent_id='.$parent_id));
            }else{
                redirect(admin_url('clients/client/'.$clientid.'?group=client_document'));
            }
    ...
    public function edit_folder()
    {
        ...
        if(!empty($_POST)){
            ...
            $folder_name = $client_document['folder_name']; 
                ...
                $this->load->model('home_model');   
                $update = $this->home_model->update('tblclientdocuments', $ad_data,array('id'=>$id));  
    ...
    public function upload_file()
    {
        ...
        if(!empty($_POST)){
            ...
            $ff =  client_document_attachments($doc_id);  

        
            set_alert('success', 'File Uploaded Successfully');

            if($parent_id > 0){
                redirect(admin_url('clients/client/'.$client_id.'?group=client_document&parent_id='.$parent_id));
            }else{
                redirect(admin_url('clients/client/'.$client_id.'?group=client_document'));
            }
    ...
    public function delete_file($id,$parent_id="",$client_id="")
    { 
    ...
    public function challan_search() {
        if(!empty($_POST)){
           ...
           $date_range = get_date_range($range); 
           redirect(admin_url('clients/client/'.$clientid.'?group=chalan&from_date='.$date_range['from_date'].'&to_date='.$date_range['to_date'].'&range='.$range.'&f_date='.$f_date.'&t_date='.$t_date));
        }
    }
    ...
    public function expense_search() {
        if(!empty($_POST)){
           ...
           $date_range = get_date_range($range); 
           redirect(admin_url('clients/client/'.$clientid.'?group=expenses&from_date='.$date_range['from_date'].'&to_date='.$date_range['to_date'].'&range='.$range.'&f_date='.$f_date.'&t_date='.$t_date));
        }
    }

    public function dn_search() {
        if(!empty($_POST)){
            extract($this->input->post());

           $date_range = get_date_range($range); 
           redirect(admin_url('clients/client/'.$clientid.'?group=debit_note&from_date='.$date_range['from_date'].'&to_date='.$date_range['to_date'].'&range='.$range.'&f_date='.$f_date.'&t_date='.$t_date));
        }
    }

    public function payment_dn_search() {
        if(!empty($_POST)){
            extract($this->input->post());

           $date_range = get_date_range($range); 
           redirect(admin_url('clients/client/'.$clientid.'?group=payment_debit_note&from_date='.$date_range['from_date'].'&to_date='.$date_range['to_date'].'&range='.$range.'&f_date='.$f_date.'&t_date='.$t_date));
        }
    }
