...
class Clients extends Clients_controller {
    ...
    public function get_calendar_data() {
        ...
        $data = $this->utilities_model->get_calendar_data(
                $this->input->get('start'), $this->input->get('end'), get_user_id_by_contact_id(get_contact_user_id()), get_contact_user_id()
        );
            ...
            switch ($action) {
                ...
                case 'edit_task':
                    ...
                    if (!$task_id) {
                        ...
                        if ($project->settings->edit_tasks == 1 && total_rows('tblstafftasks', ['is_added_from_contact' => 1, 'addedfrom' => get_contact_user_id()]) > 0) {
                            $affectedRows = 0;
    ...
    public function register() {
        ...
        if (is_gdpr() && get_option('gdpr_enable_terms_and_conditions') == 1) {
            $this->form_validation->set_rules(
                    'accept_terms_and_conditions', _l('terms_and_conditions'), 'required', ['required' => _l('terms_and_conditions_validation')]
            );
        }
