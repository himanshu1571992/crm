    ...
    public function get_challan_info(){
        ...
        if(!empty($id) && !empty($user_id)){
            ...
            if($challanprocess_info->type == 1){
                ...
                $doc_id = $challan_info->id;
                $challan_number = $challan_info->chalanno;
                ...
                $created_date = _d($challan_info->datecreated);
            
            }elseif($challanprocess_info->type == 2){
                $challan_for='Purchase Order';

                $po_info = $this->db->query("SELECT * FROM tblpurchaseorder WHERE id = '".$challanprocess_info->po_id."'")->row();
                $challan_number = (is_numeric($po_info->number)) ? 'PO-' . $po_info->number : $po_info->number;
                
                $shipfrom_info = get_vendor_info($po_info->vendor_id);

                if($po_info->source_type == 1){
                    $shipto_info = get_warehouse_info($po_info->warehouse_id);
                }else{
                    $shipto_info = get_ship_to_array($po_info->site_id);
                }
                $vendor_info = get_vendor_info($po_info->vendor_id);
                $client_name = $vendor_info['name'];

                $doc_id = $po_info->id;
                $created_date = _d($po_info->date);
                $pdf_link = base_url('challan_API/po_pdf/'.$po_info->id);

            }elseif($challanprocess_info->type == 3){
                $challan_for='Job Work';
                $job_work_info = $this->db->query("SELECT * FROM tbljobdelivarychallan WHERE id = '".$challanprocess_info->job_work_id."'")->row();
                $challan_number = 'JDC-' . str_pad($job_work_info->id, 4, '0', STR_PAD_LEFT);
                if($challanprocess_info->for == 2){
                    $shipfrom_info = get_vendor_info($job_work_info->vendor_id);
                    $shipto_info = get_branch_info($job_work_info->billing_branch_id);
                }elseif($challanprocess_info->for == 1){
                    
                    $shipfrom_info = get_branch_info($job_work_info->billing_branch_id);
                    $shipto_info = get_vendor_info($job_work_info->vendor_id);
                }
                $vendor_info = get_vendor_info($job_work_info->vendor_id);
                $client_name = $vendor_info['name'];

                $doc_id = $job_work_info->id;
                $created_date = _d($job_work_info->date);
                $pdf_link = base_url('challan_API/job_pdf/'.$job_work_info->id);
            }
            ...
            $arr = array(
                'id' => $doc_id,
                'chalanno' => $challan_number,
                'site_person' => $site_person,
                'site_person_number' => $site_person_number,
                ...
                'delivery_person_name' => ($challanprocess_info->delivery_person_id > 0 ) ? get_employee_name($challanprocess_info->delivery_person_id) : "",
                'driver_id' => $challanprocess_info->driver_id,
                'scaffolder_id' => $challanprocess_info->scaffolder_id,
                'scaffolder_name' => ($challanprocess_info->scaffolder_id > 0) ? get_employee_name($challanprocess_info->scaffolder_id) : '',
    ...
    public function getChallanForTrip(){
        ...
        if(!empty($trip_id)){
            ...
            if(!empty($last_challan_arr)){
                foreach ($last_challan_arr as $process_id) {
                    ...
                    if($challanprocess_info->type == 1){
                        $getchallandetails = $this->db->query("SELECT * FROM tblchalanmst WHERE `id` = '".$challanprocess_info->chalan_id."'")->row();
                        ...
                        $challan_type = '';
                        if(!empty($getchallandetails) && $getchallandetails->service_type == 1){
                            $challan_type = 'For Rent';
                        }elseif(!empty($getchallandetails) && $getchallandetails->service_type == 2){
                            $challan_type = 'For Sales';
                        }
                    ...
                    }elseif($challanprocess_info->type == 2 || $challanprocess_info->type == 3){

                        if($challanprocess_info->type == 2){
                            $document_info = $this->db->query("SELECT * FROM tblpurchaseorder WHERE id = '".$challanprocess_info->po_id."'")->row();
                            $challan_number = (is_numeric($document_info->number)) ? 'PO-' . $document_info->number : $document_info->number;
                            $shipfrom_info = get_vendor_info($document_info->vendor_id);

                            if($document_info->source_type == 1){
                                $shipto_info = get_warehouse_info($document_info->warehouse_id);
                            }else{
                                $shipto_info = get_ship_to_array($document_info->site_id);
                            }
                        }else{
                            $document_info = $this->db->query("SELECT * FROM tbljobdelivarychallan WHERE id = '".$challanprocess_info->job_work_id."'")->row();
                            $challan_number = 'JDC-' . str_pad($document_info->id, 4, '0', STR_PAD_LEFT);
                            if($challanprocess_info->for == 2){
                                $shipfrom_info = get_vendor_info($document_info->vendor_id);
                                $shipto_info = get_branch_info($document_info->billing_branch_id);
                            }elseif($challanprocess_info->for == 1){
                                
                                $shipfrom_info = get_branch_info($document_info->billing_branch_id);
                                $shipto_info = get_vendor_info($document_info->vendor_id);
                            }
                        }                       

                        $vendor_info = get_vendor_info($document_info->vendor_id);
                        $client_name = $vendor_info['name'];
                    }
                    ...
                    $output[] = array(
                        ...
                        "po_id" => $challanprocess_info->po_id,
                        "job_work_id" => $challanprocess_info->job_work_id,
                        "challan_for_id" => $challanprocess_info->for,
                        "type_id" => $challanprocess_info->type,
                        "type" => $type,
        ...
        if (!empty($getchallan_data)){
            foreach ($getchallan_data as $value) {
                ...
                if($value->type == 1){
                    $getchallandetails = $this->db->query("SELECT * FROM tblchalanmst WHERE `id` = '".$value->chalan_id."'")->row();
                    $challan_number = (!empty($getchallandetails)) ? $getchallandetails->chalanno : "";
                    $client_name = (!empty($getchallandetails) && $getchallandetails->clientid > 0) ? client_info($getchallandetails->clientid)->client_branch_name : "";
                    ...
                    if($value->type == 2){
                        ...
                        $document_info = $this->db->query("SELECT * FROM tblpurchaseorder WHERE id = '".$value->po_id."'")->row();
                        $challan_number = (is_numeric($document_info->number)) ? 'PO-' . $document_info->number : $document_info->number;
                        ...
                        $document_info = $this->db->query("SELECT * FROM tbljobdelivarychallan WHERE id = '".$value->job_work_id."'")->row();
                        $challan_number = 'JDC-' . str_pad($document_info->id, 4, '0', STR_PAD_LEFT);
                        if($value->for == 2){
                            $shipfrom_info = get_vendor_info($document_info->vendor_id);
                            $shipto_info = get_branch_info($document_info->billing_branch_id);
                        ...
                        foreach($challan_arr as $process_id){                            
                            ...
                            if(!empty($process_info)){
                                ...
                                if($process_info->type == 1){
                                    ...
                                    $type = 'Purchase Order';
                                    $po_info = $this->db->query("SELECT * FROM tblpurchaseorder WHERE id = '".$process_info->po_id."'")->row();
                                    $challan_number = (is_numeric($po_info->number)) ? 'PO-' . $po_info->number : $po_info->number;
                                    ...
                                    $type = 'Job Work';
                                    $job_work_info = $this->db->query("SELECT * FROM tbljobdelivarychallan WHERE id = '".$process_info->job_work_id."'")->row();
                                    $challan_number = 'JDC-' . str_pad($job_work_info->id, 4, '0', STR_PAD_LEFT);
                                    if($process_info->for == 2){
                                        $shipfrom_info = get_vendor_info($job_work_info->vendor_id);
                                        $shipto_info = get_branch_info($job_work_info->billing_branch_id);
                                ...
                                $challan_data[] = array(
                                    ...
                                    "po_id" => $process_info->po_id,
                                    "job_work_id" => $process_info->job_work_id,
                                    "challan_for_id" => $process_info->for,
                                    "type_id" => $process_info->type,
                                    "type" => $type,
            ...
            if(!empty($challan_arr)){
                foreach($challan_arr as $process_id){
                    ...
                    if($sval->type == 1){
                        ...
                        $type = 'Purchase Order';
                        $po_info = $this->db->query("SELECT * FROM tblpurchaseorder WHERE id = '".$sval->po_id."'")->row();
                        $challan_number = (is_numeric($po_info->number)) ? 'PO-' . $po_info->number : $po_info->number;
                        ...
                        $type = 'Job Work';
                        $job_work_info = $this->db->query("SELECT * FROM tbljobdelivarychallan WHERE id = '".$sval->job_work_id."'")->row();
                        $challan_number = 'JDC-' . str_pad($job_work_info->id, 4, '0', STR_PAD_LEFT);
                        if($sval->for == 2){
                            $shipfrom_info = get_vendor_info($job_work_info->vendor_id);
                            $shipto_info = get_branch_info($job_work_info->billing_branch_id);
