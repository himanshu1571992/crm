...
class Tasks_model extends CRM_Model
{
    ...
    public function add($data, $clientRequest = false)
    {
        ...
        if (isset($data['repeat_every']) && $data['repeat_every'] != '') {
            ...
            if ($data['repeat_every'] == 'custom') {
                ...
                if($data['recurring_type']==null)
                {$_temp                    = explode('-', $data['repeat_every']);
                $data['recurring_type']   = $_temp[1];
                $data['repeat_every']     = $_temp[0];}
                else
                {
                $data['recurring_type']   = $data['recurring_type'];
                $data['repeat_every']     = $data['repeat_every'];
                }
        ...
        if ($insert_id) {
            ...
            if ($clientRequest == false) {
				...
				if(count($stockdata)==0)
			    {
					if ($new_task_auto_assign_creator == true) {
                        ...
                        if(get_staff_user_id()==0)
                        {
                            $put_staffid=$data['addedfrom'];
                        }
                        else
                        {
                            $put_staffid=get_staff_user_id();
                        }

						$this->db->insert('tblstafftaskassignees', [
							'taskid'        => $insert_id,
							'staffid'       =>$put_staffid,
							'assigned_from' => $put_staffid,
						]);
			...
			if(count($stockdata)!=0)
			{
				...
				foreach($stockdata as $singlestockdata)
				{
					$pcdata['component_id']=$singlestockdata['component_id'];
					$pcdata['requiredqty']=$singlestockdata['requiredqty'];
					$pcdata['availableqty']=$singlestockdata['availableqty'];
					$pcdata['produceqty']=$singlestockdata['remainingqty'];
					$pcdata['task_id']=$insert_id;
					$pcdata['dateadded']= date('Y-m-d H:i:s');
					$this->db->insert('tblproducecomponentstock', $pcdata);
					//echo $this->db->last_query();exit;
				}
				 return 'stock,'.$insert_id;
			}
    ...
    public function update($data, $id, $clientRequest = false)
    {
        ...
        {
            $this->db->where('id', $id);
            return $this->db->update('tblstafftasks', $data);
    ...
    public function add_task_assignees($data, $cronOrIntegration = false, $clientRequest = false)
    {
        ...
        if ($cronOrIntegration) {
            ...
            if (get_staff_user_id() == 0) {
                 $assignData['assigned_from']=$data['assigned_from'];
            }
            else{
            ...
            foreach ($comments as $key => $comment) {
                ...
                foreach ($allAttachments as $attachment) {
                    if ($comment['id'] == $attachment['task_comment_id']) {
                        $file_count++;
                        $comments[$key]['file_count']=$file_count;
    ...
    public function edit_comment($data)
    {
        ...
        $comment = $this->db->get('tblstafftaskcomments')->row();
        if ($comment->staffid == get_staff_user_id() || has_permission('tasks', '', 'edit') || $comment->contact_id == get_contact_user_id()|| $comment->staffid == $data['staffid']) {
