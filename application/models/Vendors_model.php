...
class Vendors_model extends CRM_Model
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(['vendor_vault_entries_model', 'vendor_groups_model', 'statement_model']);
    }

    /**
     * Get vendor object based on passed vendorid if not passed vendorid return array of all vendors
    ...
    {
        $this->db->select(implode(',', prefixed_table_fields_array('tblvendorbranch')) . ',' . get_sql_select_vendor_branch_company());

        $this->db->join('tblcountries', 'tblcountries.country_id = tblvendorbranch.country', 'left');
        $this->db->join('tblvendorcontacts', 'tblvendorcontacts.userid = tblvendorbranch.userid AND is_primary = 1', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where('tblvendorbranch.userid', $id);
            $vendor = $this->db->get('tblvendorbranch')->row();
        ...
        return $this->db->get('tblvendorbranch')->result_array();
    }
	
	
	public function add($data, $vendor_or_lead_convert_request = false)
    {
        $contact_data = [];
        ...
        $hook_data = do_action('before_vendor_added', ['data' => $data]);
        $data      = $hook_data['data'];
		
        $this->db->insert('tblvendorbranch', $data);
        ...
        if ($userid) {
            ...
            if ($vendor_or_lead_convert_request == true) {
                $contact_id = $this->add_contact($contact_data, $userid, $vendor_or_lead_convert_request);
            }
            if (isset($groups_in)) {
            ...
            do_action('after_vendor_added', $userid);
            $log = 'ID: ' . $userid;
            ...
            logActivity('New Vendor Branch Created [' . $log . ']', $isStaff);
        }
    ...
    public function get_contacts($customer_id = '', $where = ['active' => 1])
    {
        ...
        return $this->db->get('tblvendorcontacts')->result_array();
    }
    ...
    public function get_contact($id)
    {
        ...
        return $this->db->get('tblvendorcontacts')->row();
    }

    /**
     * @param array $_POST data
     * @param vendor_request is this request from the customer area
     ...
     */
    public function update($data, $id, $vendor_request = false)
    {
        ...
        $_data = do_action('before_vendor_updated', [
            'userid' => $id,
            'data'   => $data,
        ]);
		unset($data['DataTables_Table_0_length']);
        $this->db->where('userid', $id);
        $this->db->update('tblvendorbranch', $data);
        ...
        if ($this->vendor_groups_model->sync_customer_groups($id, $groups_in)) {
            $affectedRows++;
        }

        if ($affectedRows > 0) {
            do_action('after_vendor_updated', $id);
     ...
     * @param  mixed  $id             contact id
     * @param  boolean $vendor_request is request from customers area
     * @return mixed
     */
    public function update_contact($data, $id, $vendor_request = false)
    {
        $affectedRows = 0;
        ...
        $this->db->where('id', $id);
        $this->db->update('tblvendorcontacts', $data);
            ...
            if (sizeof($customer_permissions) > 0) {
                foreach ($customer_permissions as $customer_permission) {
                    if (!in_array($customer_permission['permission_id'], $permissions)) {
                        ...
                        $this->db->delete('tblvendorcontactpermissions');
                        if ($this->db->affected_rows() > 0) {
                            $affectedRows++;
                        }
                    }
                ...
                foreach ($permissions as $permission) {
                    ...
                    $_exists = $this->db->get('tblvendorcontactpermissions')->row();
                    if (!$_exists) {
                        $this->db->insert('tblvendorcontactpermissions', [
                            'userid'        => $id,
    ...
    public function add_contact($data, $customer_id, $not_manual_request = false)
    {
        ...
        if (isset($data['donotsendwelcomeemail'])) {
            ...
            // If vendor register set this auto contact as primary
            $data['is_primary'] = 1;
        }
        ...
        $this->db->insert('tblvendorcontacts', $data);
        ...
        if ($contact_id) {
            ...
            foreach ($permissions as $permission) {
                ...
                if ($not_manual_request == true) {
                    if ($permission == 6) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblvendorcontacts', ['project_emails' => 1, 'task_emails' => 1]);
                    } elseif ($permission == 3) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblvendorcontacts', ['contract_emails' => 1]);
                    } elseif ($permission == 2) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblvendorcontacts', ['estimate_emails' => 1]);
                    } elseif ($permission == 1) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblvendorcontacts', ['invoice_emails' => 1, 'credit_note_emails' => 1]);
                    } elseif ($permission == 5) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblvendorcontacts', ['ticket_emails' => 1]);
                    }
                }
            }
            ...
            if ($send_welcome_email == true) {
                ...
                $merge_fields = [];
                $merge_fields = array_merge($merge_fields, get_vendor_contact_merge_fields($data['userid'], $contact_id, $password_before_hash));
    ...
    public function update_company_details($data, $id)
    {
        ...
        if ($affectedRows > 0) {
            ...
            logActivity('Customer Info Updated From Vendors Area [ID: ' . $id . ']');
     ...
     * @param  integer ID
     * @return boolean
     * Delete vendor, also deleting rows from, dismissed vendor announcements, ticket replies, tickets, autologin, user notes
     */
    public function delete($id)
    {
        ...
        $this->db->where('userid', $id);
        $this->db->delete('tblvendorbranch');
        if ($this->db->affected_rows() > 0) {
            $affectedRows++;
            // Delete all user contacts
            $this->db->where('userid', $id);
            $contacts = $this->db->get('tblvendorcontacts')->result_array();
            ...
            $this->db->where('vendor_id', $id);
            $this->db->update('tblleads', ['vendor_id' => 0]);
        ...
        if ($affectedRows > 0) {
            do_action('after_vendor_deleted', $id);
            ...
            logActivity('Vendor Branch Deleted [ID: ' . $id . ']');
    ...
    {
        $this->db->where('id', $id);
        $result      = $this->db->get('tblvendorcontacts')->row();
        ...
        $this->db->where('id', $id);
        $this->db->delete('tblvendorcontacts');
    ...
    public function get_customer_default_currency($id)
    {
        ...
        $this->db->where('userid', $id);
        $result = $this->db->get('tblvendorbranch')->row();
        if ($result) {
    ...
    {
        $this->db->where('userid', $id);
        $this->db->update('tblvendorbranch', [
            'active' => $status,
        ]);
     ...
     * @return mixed
     * Change contact password, used from vendor area
     */
    public function change_contact_password($data)
    {
        ...
        $vendor = $this->db->get('tblvendorcontacts')->row();
        $this->load->helper('phpass');
        $hasher = new PasswordHash(PHPASS_HASH_STRENGTH, PHPASS_HASH_PORTABLE);
        if (!$hasher->CheckPassword($data['oldpassword'], $vendor->password)) {
            return [
        ...
        $this->db->where('id', get_contact_user_id());
        $this->db->update('tblvendorcontacts', $update_data);
        if ($this->db->affected_rows() > 0) {
    ...
    {
        return $this->vendor_groups_model->get_customer_groups($id);
    }
    ...
    {
        return $this->vendor_groups_model->get_groups($id);
    }
    ...
    {
        return $this->vendor_groups_model->delete($id);
    }
    ...
    {
        return $this->vendor_groups_model->add($data);
    }
    ...
    {
        return $this->vendor_groups_model->edit($data);
    }
    ...
    */
    ...
    {
        return $this->vendor_vault_entries_model->create($data, $customer_id);
    }
    ...
    {
        return $this->vendor_vault_entries_model->update($id, $data);
    }
    ...
    {
        return $this->vendor_vault_entries_model->delete($id);
    }
    ...
    {
        return $this->vendor_vault_entries_model->get_by_customer_id($customer_id, $where);
    }
    ...
    {
        return $this->vendor_vault_entries_model->get($id);
    }
    ...
    */
     ...
     * @param  mixed $vendor_id  the customer id
     * @return boolean
     */
    public function require_confirmation($vendor_id)
    {
        $contact_id = get_primary_contact_user_id($vendor_id);
        $this->db->where('userid', $vendor_id);
        $this->db->update('tblvendorbranch', ['active' => 0, 'registration_confirmed' => 0]);

        $this->db->where('id', $contact_id);
        $this->db->update('tblvendorcontacts', ['active' => 0]);
    ...
    public function confirm_registration($vendor_id)
    {
        $contact_id = get_primary_contact_user_id($vendor_id);
        $this->db->where('userid', $vendor_id);
        $this->db->update('tblvendorbranch', ['active' => 1, 'registration_confirmed' => 1]);

        $this->db->where('id', $contact_id);
        $this->db->update('tblvendorcontacts', ['active' => 1]);
        ...
        if ($contact) {
            $this->load->model('emails_model');
            $merge_fields = [];
            $merge_fields = array_merge($merge_fields, get_vendor_contact_merge_fields($vendor_id, $contact_id));
            $this->emails_model->send_email_template('vendor-registration-confirmed', $contact->email, $merge_fields);
    ...
    public function get_vendors_distinct_countries()
    {
       return $this->db->query('SELECT DISTINCT(country_id), short_name FROM tblvendorbranch JOIN tblcountries ON tblcountries.country_id=tblvendorbranch.country')->result_array();
    }
	...
	public function deletedocument($id) {
        ...
        $this->db->where('id', $id);
        $this->db->delete('tblvendordocument');  

        if ($this->db->affected_rows() > 0) {
            logActivity('Document Deleted [ID: ' . $id . ']');
            return true;
        }
        return false;
    }
