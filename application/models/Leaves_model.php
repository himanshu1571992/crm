...
class Leaves_model extends CRM_Model	 
{
   
	private $table_name = "tblleavescategories";
   public function __construct()
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            logActivity('New Leave Added [' . $insert_id . ']');
   ...
   public function delete($id, $simpleDelete = false)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('tblleaves');

        if ($this->db->affected_rows() > 0) {
           

            $this->db->where('leave_id', $id);
            $this->db->delete('tblleaveapproval');

            logActivity('Leave Deleted [' . $id . ']');
    ...
    public function get_category($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get('tblleavescategories')->row();
        }
        $this->db->order_by('order', 'asc');

        return $this->db->get('tblleavescategories')->result_array();
    }
    ...
    public function add_category($data)
    {
	  ...
	  // $data['description'] = nl2br($data['description']);
        $this->db->insert('tblleavescategories', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    public function update_category($data, $id)
    {
         $data['updated_at'] = date("Y-m-d H:i:s");
        $this->db->where('id', $id);
        $this->db->update('tblleavescategories', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_category($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('tblleavescategories');
        if ($this->db->affected_rows() > 0) {
