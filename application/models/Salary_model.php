...
class Salary_model extends CRM_Model	 
{
   ...
   public function __construct()
	 ...
	 public function get_deduction($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get('tblsalarydeduction')->row();
        }
        $this->db->order_by('order', 'asc');

        return $this->db->get('tblsalarydeduction')->result_array();
    }
    ...
    public function add_deduction($data)
    {
        ...
        $this->db->insert('tblsalarydeduction', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            logActivity('New deduction Added [ID: ' . $insert_id . ']');
    ...
    public function update_deduction($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('tblsalarydeduction', $data);
        if ($this->db->affected_rows() > 0) {
            logActivity('Event Updated [ID: ' . $id . ']');
	...
	public function edit_deduction($data, $id) {
