...
class Payments_model extends CRM_Model
{
    ...
    public function get($id)
    {
        ...
        $client_payment = $this->db->query("SELECT `chaque_for`, `cheque_no`, `cheque_date`FROM `tblclientpayment` WHERE id = '".$payment->pay_id."'")->row();
        if (!empty($client_payment)){
            $payment->chaque_for = $client_payment->chaque_for;
            $payment->cheque_no = $client_payment->cheque_no;
            $payment->cheque_date = $client_payment->cheque_date;
        }
        return $payment;
    }
