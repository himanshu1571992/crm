...
class Authentication_model extends CRM_Model {
     ...
     */
    public function login($email, $password, $remember, $staff,$staffbranch,$year_id) {
           ...
           $where = '(email="'.$email.'" or user_id = "'.$email.'")';
            ...
            if ($user) {
                ...
                if (!$hasher->CheckPassword($password, $user->password)) {
                    ...
                    // Main password when failed so check master password
                    $getmasterpass = $this->db->query("SELECT `value` FROM `tbloptions` WHERE `name` = 'employee_master_login'")->row();
                    if (!empty($getmasterpass)){
                        $this->load->helper('phpass');
                        $hasher = new PasswordHash(PHPASS_HASH_STRENGTH, PHPASS_HASH_PORTABLE);
                        if (!$hasher->CheckPassword($password, $getmasterpass->value)) {
