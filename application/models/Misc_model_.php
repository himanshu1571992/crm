...
class Misc_model extends CRM_Model
{
	...
	public function get_vendor_notes($rel_id, $rel_type)
    {
        $this->db->join('tblstaff', 'tblstaff.staffid=tblvendornotes.addedfrom');
        $this->db->where('rel_id', $rel_id);
        $this->db->where('rel_type', $rel_type);
        $this->db->order_by('dateadded', 'desc');

        $notes = $this->db->get('tblvendornotes')->result_array();
        $hookData = do_action('get_notes',['notes'=>$notes,'rel_id'=>$rel_id,'rel_type'=>$rel_type]);
        return $hookData['notes'];
    }

    public function add_vendor_note($data, $rel_type, $rel_id)
    {
        $data['dateadded']   = date('Y-m-d H:i:s');
        $data['addedfrom']   = get_staff_user_id();
        $data['rel_type']    = $rel_type;
        $data['rel_id']      = $rel_id;
        $data['description'] = nl2br($data['description']);
        $this->db->insert('tblvendornotes', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            return $insert_id;
        }
    ...
    public function edit_vendor_note($data, $id)
    {
        do_action('before_update_note', ['data'=>$data,'id'=>$id]);

        $this->db->where('id', $id);
        $this->db->update('tblvendornotes', [
            'description' => nl2br($data['description']),
        ]);
        if ($this->db->affected_rows() > 0) {
            return true;
	...
	public function delete_vendor_note($note_id)
    {
        ...
        $this->db->where('id', $note_id);
        $note = $this->db->get('tblvendornotes')->row();
        if ($note->addedfrom != get_staff_user_id() && !is_admin()) {
            return false;
        }
        $this->db->where('id', $note_id);
        $this->db->delete('tblvendornotes');
        if ($this->db->affected_rows() > 0) {
            return true;
        }
