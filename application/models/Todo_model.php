...
class Todo_model extends CRM_Model
{
    ...
    /*public function update_todo_items_order($data)
    {
        for ($i = 0; $i < count($data['data']); $i++) {
            ...
            if($data['data'][$i][3] == 1){
               $this->db->update('tblpaymentfollowupclients', $update); 
            }elseif($data['data'][$i][3] == 2){
               $this->db->update('tblleadfollowup', $update); 
            }
    ...
    /*public function change_todo_status($id, $status)
    {
        ...
        $this->db->where('staffid', get_staff_user_id());
        $date = date('Y-m-d H:i:s');
        $this->db->update('tblpaymentfollowupclients', [
            'finished'     => $status,
            'datefinished' => $date,
        ]);
        if ($this->db->affected_rows() > 0) {
    ...
    public function change_lead_status($id, $status)
    {
        ...
        $this->db->where('staffid', get_staff_user_id());
        $date = date('Y-m-d H:i:s');
        $this->db->update('tblleadfollowup', [
            'finished'     => $status,
            'datefinished' => $date,
        ]);
        if ($this->db->affected_rows() > 0) {
