...
class Reports_model extends CRM_Model
{
    ...
    public function getIndiamart(){
        $result = $this->callindia_mart();
        if (!empty($result)){
            $response = json_decode($result);
            foreach ($response as $value) {
                if (!property_exists($value, 'Error_Message')){
                    $data["unique_id"] = $value->QUERY_ID;
                    $data["source_type"] = $value->QTYPE;
                    $data["customer_name"] = $value->SENDERNAME;
                    $data["email"] = $value->SENDEREMAIL;
                    $data["mobile"] = $value->MOB;
                    $data["company_name"] = $value->GLUSR_USR_COMPANYNAME;
                    $data["address"] = $value->ENQ_ADDRESS;
                    $data["city"] = $value->ENQ_CITY;
                    $data["state"] = $value->ENQ_STATE;
                    $data["country"] = $value->COUNTRY_ISO;
                    $data["product_name"] = $value->PRODUCT_NAME;
                    $data["message"] = $value->ENQ_MESSAGE;
                    $data["enquiry_type"] = $value->QUERY_MODID;
                    $data["call_duration"] = $value->ENQ_CALL_DURATION;
                    $data["receiver_number"] = $value->ENQ_RECEIVER_MOB;
                    $data["alternate_email"] = $value->EMAIL_ALT;
                    $data["alternate_mobile"] = $value->MOBILE_ALT;
                    $data["date"] = date("Y-m-d", strtotime($value->DATE_TIME_RE));
                    $data["date_time"] = date("Y-m-d H:i:s", strtotime($value->DATE_TIME_RE));
                    $data["created_at"] = date("Y-m-d H:i:s");
                    
                    $this->home_model->insert("tblindiamartclientrecord", $data);
                }
            }
        }
    }
