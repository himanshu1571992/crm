...
class Clients_model extends CRM_Model
{
    ...
    /*public function get($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            $client = $this->db->get('tblclientbranch')->row();
    ...
    public function get($id = '', $where = [])
    {
        //$this->db->select(implode(',', prefixed_table_fields_array('tblclientbranch')) . ',' . get_sql_select_client_branch_company());

        $this->db->join('tblcountries', 'tblcountries.country_id = tblclientbranch.country', 'left');
        $this->db->join('tblcontacts', 'tblcontacts.userid = tblclientbranch.userid AND is_primary = 1', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where('tblclientbranch.userid', $id);
            $client = $this->db->get('tblclientbranch')->row();
	...
	public function getcust($id = '', $where = [])
    {
    ...
    public function add($data, $client_or_lead_convert_request = false)
    {
        ...
        if ($userid) {
            ...
            logActivity('New Client Branch Created [' . $log . ']', $isStaff);
        }
    ...
    public function update($data, $id, $client_request = false)
    {
        ...
        $this->db->update('tblclientbranch', $data);
    ...
    public function delete($id)
    {
        ...
        $this->db->where('userid', $id);
        $this->db->delete('tblclientbranch');
        if ($this->db->affected_rows() > 0) {
            $affectedRows++;
        ...
        if ($affectedRows > 0) {
            ...
            logActivity('Client Branch Deleted [ID: ' . $id . ']');
    ...
    public function get_customer_default_currency($id)
    {
        ...
        $this->db->where('userid', $id);
        $result = $this->db->get('tblclientbranch')->row();
        if ($result) {
    ...
    */
    ...
    public function require_confirmation($client_id)
    {
        ...
        $this->db->update('tblclientbranch', ['active' => 0, 'registration_confirmed' => 0]);
    ...
    public function confirm_registration($client_id)
    {
        $contact_id = get_primary_contact_user_id($client_id);
        $this->db->where('userid', $client_id);
        $this->db->update('tblclientbranch', ['active' => 1, 'registration_confirmed' => 1]);
    ...
    {
        return $this->db->query('SELECT DISTINCT(country_id), short_name FROM tblclientbranch JOIN tblcountries ON tblcountries.country_id=tblclientbranch.country')->result_array();
    }
