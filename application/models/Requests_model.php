...
class Requests_model extends CRM_Model	 
{
   
	private $table_name = "tblrequestscategories";
   public function __construct()
    ...
    public function add($data)
    {
        ...
        $this->db->insert('tblrequests', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
           
            logActivity('New Request Added [' . $insert_id . ']');
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('tblrequests', $data);
        if ($this->db->affected_rows() > 0) {
            logActivity('Request Updated [' . $id . ']');
            $affectedRows++;
        }
        if ($affectedRows > 0) {
            return true;
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            logActivity('Request Deleted [' . $id . ']');
    ...
    public function get_category($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get('tblrequestscategories')->row();
        }
        $this->db->order_by('order', 'asc');

        return $this->db->get('tblrequestscategories')->result_array();
    }
	
	public function get_tenues($id = '')
    {
        if (is_numeric($id)) {
            $this->db->where('id', $id);

            return $this->db->get('tblloantenues')->row();
        }
        $this->db->order_by('order', 'asc');

        return $this->db->get('tblloantenues')->result_array();
    }
    ...
    public function add_category($data)
    {
	  ...
	  // $data['description'] = nl2br($data['description']);
        $this->db->insert('tblrequestscategories', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    public function update_category($data, $id)
    {
         $data['updated_at'] = date("Y-m-d H:i:s");
        $this->db->where('id', $id);
        $this->db->update('tblrequestscategories', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_category($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('tblrequestscategories');
        if ($this->db->affected_rows() > 0) {
	...
	public function update_tenue($data, $id)
    {
         $data['updated_at'] = date("Y-m-d H:i:s");
        $this->db->where('id', $id);
        $this->db->update('tblloantenues', $data);
        if ($this->db->affected_rows() > 0) {
            logActivity('Requests Tenue Updated [ID: ' . $id . ']');
	...
	public function add_tenue($data)
    {
	  ...
	  // $data['description'] = nl2br($data['description']);
        ...
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            logActivity('New Requests Tenue Added [ID: ' . $insert_id . ']');
	...
	public function edit_tenue($data, $id) {
