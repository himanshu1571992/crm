...
$config['smtp_host']    = trim(get_staffemail('smtp_host'));

if (get_staffemail('smtp_username') == '') {
    $config['smtp_user']    = trim(get_staffemail('smtp_email'));
} else {
    $config['smtp_user']    = trim(get_staffemail('smtp_username'));
}

$config['smtp_pass']    = get_instance()->encryption->decrypt(get_staffemail('smtp_password'));

$config['smtp_port']    = trim(get_staffemail('smtp_port'));

$config['smtp_timeout'] = 30;

$config['smtp_crypto'] = get_staffemail('smtp_encryption');
...
$charset = strtoupper(get_staffemail('smtp_email_charset'));
