...
<div class="modal-body">
   <div class="row">
      ...
      <?php if($task->billed == 0){
		 ...
		 $componnetstock=$this->db->query("SELECT cs.`requiredqty`,cs.`availableqty`,cs.`produceqty`,c.id,c.`name` FROM `tblproducecomponentstock` cs LEFT JOIN `tblcomponents` c ON cs.`component_id`=c.`id` WHERE cs.`task_id`='".$id."'")->result_array();
		 ?>
         <div class="clearfix"></div>
         <hr />
		 <?php
		 ...
		 <table class="table credite-note-items-table items table-main-credit-note-edit no-mtop" id="myproTable" style="margin-top:4% !important;">
			<thead>
				<tr>
					...
					<th width="10%" align="center">Required Qty</th>
					<th width="10%" class="qty" align="center">Available Qty</th>
					<th width="10%" align="center">Produce Qty</th>
					<th width="50%" align="center">Assign Staff & Produce Qty</th>
				</tr>
			</thead>
			<tbody class="ui-sortable" style="font-size:15px;">
		<?php
			$stockassignstaffdet=$this->db->query("SELECT s.`staffid`,s.firstname,s.`lastname` FROM `tblstafftaskassignees` sa LEFT JOIN `tblstaff` s ON sa.`staffid`=s.`staffid` WHERE `taskid`='".$id."'")->result_array();
				...
				?>
					<tr class="main" id="tr0">
						<td width="20%" align="left">
							<div class="form-group"><input type="text" readonly class="form-control" name="stockdata[<?php echo $i;?>][component_name]" value="<?php echo $singleavailabity['name']?>"><input type="hidden" name="stockdata[<?php echo $i;?>][component_id]" value="<?php echo $singleavailabity['id']?>"></div>
						</td>
						<td width="10%" align="center"><input type="text" readonly class="form-control" name="stockdata[<?php echo $i;?>][requiredqty]" value="<?php echo $singleavailabity['requiredqty']?>"></td>
						<td width="10%" align="center"><input type="text" readonly class="form-control" name="stockdata[<?php echo $i;?>][availableqty]" value="<?php echo $singleavailabity['availableqty']?>"></td>
						<td width="10%" align="center"  ><input type="text" readonly class="form-control" <?php // echo $class;?> name="stockdata[<?php echo $i;?>][remainingqty]" value="<?php echo abs($singleavailabity['produceqty']);?>"></td>
						<td width="50%" align="center"  >
						<?php
							foreach($stockassignstaffdet as $singlestockassignstaffdet)
							{
								echo '<div style="width:100%;"><div style="width:40%;float:left;margin-top:3%;">'.$singlestockassignstaffdet['firstname'].' '.$singlestockassignstaffdet['lastname'].':</div>';?>
						<div style="width:55%;float:left;margin-top:2%;"><input type="text" <?php if($singlestockassignstaffdet['staffid']!=get_staff_user_id()){echo'readonly';}?> class="form-control" <?php // echo $class;?> name="stockdata[<?php echo $i;?>][remainingqty]" value="<?php echo abs($singleavailabity['produceqty']);?>"></div>
						</div>
						<?php
							}?>
					</td>
