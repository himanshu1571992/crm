                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($product_info as $single_prod_sale_det) {
                                                    ...
                                                    if ($single_prod_sale_det["is_temp"] == 0){
                                                        if($single_prod_sale_det['hsn_code'] == 2){
                                                            $hsn_sac_code = $this->db->query("SELECT pf.field_value  FROM tblproductsfield as pf LEFT JOIN tblproductcustomfields as pcf ON pcf.id = pf.field_id where pcf.field_for = 2 and pcf.status = 1 and pf.product_id = '".$single_prod_sale_det['product_id']."' ")->row();
                                                            $hsn_sac_code = (!empty($hsn_sac_code)) ? $hsn_sac_code->field_value : '';
                                                        }else{
                                                            $hsn_sac_code = $this->db->query("SELECT pf.field_value  FROM tblproductsfield as pf LEFT JOIN tblproductcustomfields as pcf ON pcf.id = pf.field_id where pcf.field_for = 1 and pcf.status = 1 and pf.product_id = '".$single_prod_sale_det['product_id']."' ")->row();
                                                            $hsn_sac_code = (!empty($hsn_sac_code)) ? $hsn_sac_code->field_value : '';
                                                        }
                                                    }else{
                                                        if($single_prod_sale_det['hsn_code'] == 2){
                                ...
                                <div class="row" style="margin-top:2%;padding:8px;">
                                    <div class="col-md-12">
                                        <label class="col-md-6 control-label text-left">Approve/Reject Remark</label>
                                       <div class="form-group" app-field-wrapper="remark">
                                        <textarea id="remark" required="" name="remark" class="form-control" rows="4"><?php if(!empty($appvoal_info)){ echo $appvoal_info->approval_remark; }?></textarea>
                                    </div>
                                    </div>
                                </div>
                                <div class="row">
...
<?php
    $assign_info = $this->db->query("SELECT * from tblmasterapproval  where module_id = '62' and table_id = '".$purchase_info['id']."'  ")->result();
?>
             <div class="col-md-12">
                <div class="panel_s">
                    <div class="panel-body">
