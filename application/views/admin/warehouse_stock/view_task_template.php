...
<div class="modal-body">
   <div class="row">
      ...
      <?php if($task->billed == 0){
		 ...
		 $componnetstock=$this->db->query("SELECT cs.`requiredqty`,cs.`task_id`,cs.`id` as stockid,cs.`availableqty`,cs.`produceqty`,c.id,c.`name` FROM `tblproducecomponentstock` cs LEFT JOIN `tblcomponents` c ON cs.`component_id`=c.`id` WHERE cs.`task_id`='".$id."'")->result_array();
		 ?>
         
	 <?php
		 if(count($componnetstock)>0)
		 {?>
		<div class="clearfix"></div>
         <hr />
		 <table class="table credite-note-items-table items table-main-credit-note-edit no-mtop" id="myproTable" style="margin-top:4% !important;">
			<thead>
				<tr>
					<th width="20%" align="center">Component Name</th>
					<th width="15%" align="center">Required Qty</th>
					<th width="15%" class="qty" align="center">Available Qty</th>
					<th width="15%" align="center">Produce Qty</th>
					<th width="35%" align="center">Assign Staff & Produce Qty</th>
				</tr>
			</thead>
			<tbody class="ui-sortable" style="font-size:15px;">
		<?php
			$stockassignstaffdet=$this->db->query("SELECT s.`staffid`,s.firstname,s.`lastname` FROM `tblstafftaskassignees` sa LEFT JOIN `tblstaff` s ON sa.`staffid`=s.`staffid` WHERE sa.`taskid`='".$id."'")->result_array();
			
				$i=0;		
				foreach($componnetstock as $singleavailabity)
				{
					$i++;
				...
				?>
					<tr class="main" id="tr0">
						<td width="20%" align="left">
							<div class="form-group"><?php echo $singleavailabity['name']?><input type="hidden" readonly class="form-control" name="stockdata[<?php echo $i;?>][component_name]" value="<?php echo $singleavailabity['name']?>"><input type="hidden" name="stockdata[<?php echo $i;?>][component_id]" value="<?php echo $singleavailabity['id']?>"></div>
						</td>
						<td width="15%" align="center"><input type="text" readonly class="form-control" name="stockdata[<?php echo $i;?>][requiredqty]" value="<?php echo $singleavailabity['requiredqty']?>"></td>
						<td width="15%" align="center"><input type="text" readonly class="form-control" name="stockdata[<?php echo $i;?>][availableqty]" value="<?php echo $singleavailabity['availableqty']?>"></td>
						<td width="15%" align="center"  ><input type="text" class="form-control" <?php // echo $class;?> name="stockdata[<?php echo $i;?>][remainingqty]" value="<?php echo abs($singleavailabity['produceqty']);?>"></td>
						<td width="35%">
						<?php
							foreach($stockassignstaffdet as $singlestockassignstaffdet)
							{
								echo '<div style="width:40%;float:left;margin-top:3%;">'.$singlestockassignstaffdet['firstname'].' '.$singlestockassignstaffdet['lastname'].':</div>';?>
						<div style="width:60%;float:left;margin-top:2%;"><input type="text" class="form-control" readonly <?php if($singlestockassignstaffdet['staffid']!=get_staff_user_id()){echo'readonly';}?> name="stockdata[<?php echo $i;?>][remainingqty]" value="<?php echo abs($singleavailabity['produceqty']);?>"></div>
						
						<?php
							}
							if($task->addedfrom!=get_staff_user_id())
							{?>
							<button type="button" class="btn btn-info" style="margin-top:4%;" data-toggle="modal" data-target="#myModal<?php echo $singleavailabity['stockid'];?>">Update Stock</button>
							<div class="modal fade" id="myModal<?php echo $singleavailabity['stockid'];?>" role="dialog">
								<div class="modal-dialog">
								  <!-- Modal content-->
								  <div class="modal-content">
								  <?php echo form_open($this->uri->uri_string(), array('id' => 'stock-form', 'class' => 'proposal-form', 'enctype' => 'multipart/form-data')); ?>
									<input type="hidden" id="taskid" name="taskid" value="<?php echo $singleavailabity['task_id'];?>">
									<div class="modal-header">
									  <button type="button" class="close" data-dismiss="modal">&times;</button>
									  <h4 class="modal-title">Add Stock Of <?php echo $singleavailabity['name']?></h4>
									</div>
									<div class="modal-body">
									  <div class="form-group" app-field-wrapper="stockqty">
										<label for="subject" class="control-label">Stock Qty</label>
										<input type="text" id="stockqty<?php echo $singleavailabity['stockid'];?>" name="stockqty" class="form-control" required="true" >
									  </div>
									  <div class="form-group" app-field-wrapper="starttime">
										<label for="subject" class="control-label">Start Time</label>
										<input type="time" id="starttime<?php echo $singleavailabity['stockid'];?>" name="starttime" class="form-control" required="true" >
									  </div>
									  <div class="form-group" app-field-wrapper="subject">
										<label for="subject" class="control-label">End Time</label>
										<input type="time" id="endtime<?php echo $singleavailabity['stockid'];?>" name="endtime" class="form-control" required="true" >
									  </div>
									  <div class="form-group">
									<label for="enquiry_date" class="control-label">Approved By</label>
									<select onchange="staffdropdown('<?php echo $singleavailabity['stockid'];?>')" class="form-control selectpicker " multiple data-live-search="true" id="assign<?php echo $singleavailabity['stockid'];?>" name="assign[assignid][]">
									...
									<div class="modal-footer">
									  <button type="button" class="btn btn-info qtychange" value="<?php echo $singleavailabity['stockid'];?>" cid="<?php echo $singleavailabity['id']?>">Update Qty</button>
	 ...
	 <?php
		 $managetaskstock=$this->db->query("SELECT ms.*,tc.name,ts.firstname,ts.lastname FROM `tblmanagetaskstock` ms LEFT JOIN `tblcomponents` tc ON ms.`component_id`=tc.`id` LEFT JOIN `tblstaff` ts ON ts.staffid=ms.`userid` WHERE ms.`task_id`='".$id."'")->result_array();
		 if(count($managetaskstock)>0)
		 {?>
		<div class="clearfix"></div>
         <hr />
		 <h4>Complete Stock Task<h4>
		 <table class="table credite-note-items-table items table-main-credit-note-edit no-mtop" id="myproTable" style="margin-top:4% !important;">
			<thead>
				<tr>
					<th width="20%" align="center">Component Name</th>
					<th width="35%" align="center">Complete By</th>
					<th width="10%" align="center">Qty</th>
					<th class="qty" width="10%" align="center">Task Start time</th>
					<th width="10%" align="center">Task End time</th>
					<th width="15%" align="center">Approved Satus</th>
				</tr>
			</thead>
			<tbody class="ui-sortable" style="font-size:15px;">
				<?php
				foreach($managetaskstock as $singlemanagetaskstock)
				{?>
				<tr class="main" id="tr0">
					<td width="20%" align="left">
						<div class="form-group"><?php echo $singlemanagetaskstock['name'];?></div>
					</td>
					<td width="35%" align="center">
						<?php echo $singlemanagetaskstock['firstname'].' '.$singlemanagetaskstock['lastname'];?>
					</td>
					<td width="10%" align="center"><?php echo $singlemanagetaskstock['qty'];?></td>
					<td width="10%" align="center"><?php echo $singlemanagetaskstock['starttime'];?></td>
					<td width="10%" align="center"><?php echo $singlemanagetaskstock['endtime'];?></td>
					<td width="15%" align="center"><?php if($singlemanagetaskstock['status']==1){echo'<span style="margin-left: 6%;color: #fff;border-radius: 9px;padding: 8px;background: #4CAF50;"> Approved</span>';}else{echo'<span style="margin-left: 6%;color: #fff;border-radius: 9px;padding: 8px;background: #F44336;">Not Approved</span>';}?></td>
...
$('.qtychange').click(function(){
	...
	var componentid=$(this).attr('cid');
	var taskid=$('#taskid').val();
	var stockqty=$('#stockqty'+stockid).val();
	var starttime=$('#starttime'+stockid).val();
	var endtime=$('#endtime'+stockid).val();
	var url = '<?php echo base_url(); ?>admin/Stock/updatestockqty';
	var formData = $('#stock-form').serialize();
	var form = document.getElementById('stock-form');
	var data = new FormData(form);
	data.append('componentid',componentid);
	data.append('stockqty',stockqty);
...
function staffdropdown(stockid)
{
	$.each($("#assign"+stockid+" option:selected"), function(){
	   var select=$(this).val();   
	   $("optgroup."+select).children().attr('selected','selected');
	});
	$('.selectpicker').selectpicker('refresh');
	$.each($("#assign"+stockid+" option:not(:selected)"), function(){
	   var select=$(this).val();   
