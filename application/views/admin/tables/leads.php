...
$aColumns = array_merge($aColumns, ['company',
    'tblleads.email as email','tblleads.client_id as client_id', 'tblleads.client_branch_id as client_branch_id','tblleads.followup as followup',
...
$join = [
   ...
   //'LEFT JOIN tblclientbranch ON tblclientbranch.userid = tblleads.client_id',
];
...
if (!has_permission('leads', '', 'view')) {
    //array_push($where, 'AND ((SELECT count(*) as dd FROM `tblleadassignstaff` WHERE `staff_id`=' . get_staff_user_id() . ' AND `lead_id`=tblleads.id)>0 OR addedfrom = ' . get_staff_user_id() . ' OR is_public = 1)');
...
foreach ($rResult as $aRow) {
    ...
    $client_info = $this->ci->db->query("SELECT * FROM tblclientbranch WHERE userid = '".$aRow['client_branch_id']."' ")->row();

    $row[] = '<div class="checkbox"><input type="checkbox" value="' . $i . '"><label></label></div>';

    $hrefAttr = 'href="' . admin_url('leads/index/' . $aRow['id']) . '" onclick="init_lead(' . $aRow['id'] . ');return false;"';
    $row[]    = '<a ' . $hrefAttr . '>' . $i . '</a>';

    $nameRow = '<a ' . $hrefAttr . '> LEAD-'.number_series($aRow['id']) . '</a>';
    ...
    //if (!$locked) {
    if(check_permission_page(96,'edit')){
        $nameRow .= ' | <a href="' . admin_url('leads/leads/' . $aRow['id'] ) . '" >' . _l('edit') . '</a>';
    }
	...
	$row[] = date('d M Y',strtotime($aRow['enquiry_date']));
   ...
   // $row[] = $assignedOutput;
    ...
    $toggleActive = '<div class="onoffswitch">
        <input type="checkbox" data-switch-url="' . admin_url() . 'leads/change_followup_status" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['id'] . '" data-id="' . $aRow['id'] . '" ' . $checked . '>
