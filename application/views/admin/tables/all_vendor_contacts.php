...
$aColumns = array_merge($aColumns, [
    'email',
    'company',
    'tblvendorcontacts.phonenumber as phonenumber',
    'title',
    'last_login',
    'tblvendorcontacts.active as active',
]);

$sIndexColumn = 'id';
$sTable       = 'tblvendorcontacts';
$join         = ['JOIN tblclients ON tblclients.userid=tblvendorcontacts.userid'];
...
foreach ($custom_fields as $key => $field) {
    ...
    array_push($join, 'LEFT JOIN tblcustomfieldsvalues as ctable_' . $key . ' ON tblvendorcontacts.id = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
}

$where = [];

if (!has_permission('customers', '', 'view')) {
    array_push($where, 'AND tblvendorcontacts.userid IN (SELECT customer_id FROM tblcustomeradmins WHERE staff_id=' . get_staff_user_id() . ')');
}

if ($this->ci->input->post('custom_view')) {
    $filter = $this->ci->input->post('custom_view');
    if (_startsWith($filter, 'consent_')) {
        array_push($where, 'AND tblvendorcontacts.id IN (SELECT contact_id FROM tblconsents WHERE purpose_id=' . strafter($filter, 'consent_') . ' and action="opt-in" AND date IN (SELECT MAX(date) FROM tblconsents WHERE purpose_id=' . strafter($filter, 'consent_') . ' AND contact_id=tblvendorcontacts.id))');
    }
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['tblvendorcontacts.id as id', 'tblvendorcontacts.userid as userid', 'is_primary', '(SELECT count(*) FROM tblvendorcontacts c WHERE c.userid=tblvendorcontacts.userid) as total_contacts','tblclients.registration_confirmed as registration_confirmed']);
