...
if($estimate_id!='')
{
	$warehousestockdata=$this->ci->db->query("SELECT cm.id,cm.chalanno,cm.addedfrom,cm.datecreated FROM `tblcreatedchalanmst` cm LEFT JOIN tblchallanapproval ca ON cm.`id`=ca.challan_id WHERE cm.clientid='".$estimate_id."'  GROUP BY cm.id")->result_array();
}
else
{
	$warehousestockdata=$this->ci->db->query("SELECT cm.id,cm.chalanno,cm.addedfrom,cm.datecreated FROM `tblcreatedchalanmst` cm LEFT JOIN tblchallanapproval ca ON cm.`id`=ca.challan_id WHERE  (cm.`addedfrom`='".get_staff_user_id()."' OR ca.staff_id='".get_staff_user_id()."') GROUP BY cm.id")->result_array();
}
