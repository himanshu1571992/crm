...
foreach ($rResult as $aRow) {
   ...
   /* if (is_null($aRow['paymentmodeid'])) {
    ...
    if($aRow['paymentmode'] == 1){
      $payment_name = 'Cheque';
    }elseif($aRow['paymentmode'] == 2){
      $payment_name = 'NEFT';
    }elseif($aRow['paymentmode'] == 3){
      $payment_name = 'Cash';
    }
    ...
    $row[] = '<a href="' . admin_url('clients/client/' . $aRow['clientid']) . '">' . client_info($aRow['clientid'])->client_branch_name . '</a>';
