...
$total_client_contacts = total_rows('tblvendorcontacts', ['userid' => $client_id]);
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['tblvendorcontacts.id as id', 'userid', 'is_primary']);
...
foreach ($rResult as $aRow) {
    ...
    if (is_gdpr() && get_option('gdpr_enable_consent_for_contacts') == '1' && is_admin()) {
        $rowName .= ' | <a href="' . admin_url('vendors/export/' . $aRow['id']) . '">
    ...
    if (has_permission('customers', '', 'delete') || is_customer_admin($aRow['userid'])) {
        if ($aRow['is_primary'] == 0 || ($aRow['is_primary'] == 1 && $total_client_contacts == 1)) {
            $rowName .= ' | <a href="' . admin_url('vendors/delete_contact/' . $aRow['userid'] . '/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
        }
    }
