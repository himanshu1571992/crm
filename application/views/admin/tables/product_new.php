...
$join = [
    'LEFT JOIN tblunitmaster ON tblunitmaster.`id` = '.'tblproducts.`unit_2`',
];
$where  = [];
array_push($where, ' AND tblproducts.is_approved= 1');
...
//    $row[] = '<div class="checkbox"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>';
    ...
    $row[] = $name_html;
    $category_name = value_by_id('tblproductcategory',$aRow['product_cat_id'],'name');
    $row[] = $category_name;
  ...
  //  $row[] = "PROD-ID" . $aRow['id'];
    ...
    if($aRow['is_varified'] == 1){
        $is_varified = '<button disabled="" class="btn-sm btn-success">Varified</button>';
    }else{
