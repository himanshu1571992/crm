...
<div id="header">
   ...
   <nav>
      ...
      <div class="mobile-menu">
         ...
         <ul class="mobile-icon-menu">
            ...
            <!-- <li>
               <a href="<?php echo admin_url('approval/staff_notification_list'); ?>" class="dropdown-toggle notifications-icon" data-toggle="dropdown" aria-expanded="false">
                  ...
                  <?php if($current_user->total_unread_notifications > 0){ ?>
                     <span class="label icon-total-indicator bg-warning icon-notifications ttl_unread_message"><?php echo $current_user->total_unread_notifications; ?></span>
                  <?php } ?>
               </a>
            </li> -->
            ...
            <li class="icon header-newsfeed">
               <a href="<?php echo admin_url('approval/notifications'); ?>" data-toggle="tooltip" title="Notifications For Approval" data-placement="bottom"><i class="fa fa-bell-o fa-fw fa-lg" aria-hidden="true"></i>
                  <?php if(get_approval_counts() > 0){ ?>
                     <span class="label icon-total-indicator bg-warning icon-notifications"><?php echo get_approval_counts(); ?></span>
                  <?php } ?>
               </a>
            </li>
            <li class="icon header-newsfeed">
               <a href="<?php echo admin_url('approval/activity_notifications'); ?>" data-toggle="tooltip" title="Activity Tag Notifications" data-placement="bottom"><i class="fa fa-address-card fa-fw fa-lg" aria-hidden="true"></i>
                  <?php if(get_activitytag_counts() > 0){ ?>
                     <span class="label icon-total-indicator bg-warning icon-notifications"><?php echo get_activitytag_counts(); ?></span>
                  <?php } ?>
               </a>
            </li>     
            <?php } ?>
      ...
      <?php
            if(!is_mobile()){
      ?>
      <ul class="nav navbar-nav navbar-left" style="margin: 8px;">  <li ><h5 style="color: white;"><?php echo 'Branch : '.value_by_id('tblcompanybranch',get_login_branch(),'comp_branch_name'); ?> -- <?php echo 'Financial Year : '.value_by_id('tblfinancialyear',financial_year(),'slug'); ?></h5> </li></ul>
      <?php
       } ?>
      <ul class="nav navbar-nav navbar-right">
         ...
         <li class="icon header-newsfeed">
            <a href="<?php echo admin_url('approval/notifications'); ?>" data-toggle="tooltip" title="Notifications For Approval" data-placement="bottom"><i class="fa fa-users fa-fw fa-lg" aria-hidden="true"></i>
                <?php if(get_approval_counts() > 0){ ?>
                    <span class="label icon-total-indicator bg-warning icon-notifications"><?php echo get_approval_counts(); ?></span>
                 <?php } ?>
            </a>
         ...
         <!-- <li>
            <a href="<?php echo admin_url('approval/staff_notification_list'); ?>" >
               <i class="fa fa-bell fa-fw fa-lg"></i>
               <?php if($current_user->total_unread_notifications > 0){ ?>
                  <span class="label icon-total-indicator bg-warning icon-notifications ttl_unread_message"><?php echo $current_user->total_unread_notifications; ?></span>
               <?php } ?>
