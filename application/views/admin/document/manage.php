                    ...
                    <div class="panel-body">
                        ...
                        if(!empty($document_info)){
                            ...
                            <ul class="tree">
                                 <?php 
                                 if($parent_id > 0){
                                     $folder_info  = $this->db->query("SELECT * FROM tbldocuments WHERE id =  '".$parent_id."'  ")->row();
                                     echo '<li style="color:red;"><a href="'.admin_url('document/view/'.$folder_info->parent_id).'"><h4><i class="fa fa-arrow-left" aria-hidden="true"></i> '.cc($folder_info->name).'</h4></a>';
                                 }
                                 ?>
                                ...
                                <ul>
                                    ...
                                    foreach ($document_info as $key => $value) {
                                        ...
                                        if($value->type == 1){
                                            echo '<li><i class="fa fa-folder" aria-hidden="true"></i> <a href="'.admin_url('document/view/'.$value->id).'" ><b>'.cc($value->name).'</b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a val="'.$value->id.'" class="edit_folder" href="#"> <i class="fa fa-pencil" aria-hidden="true"  data-toggle="modal" data-target="#editfolder_model"></i></a></li>';    
                                        }else{
                                            $file_info  = $this->db->query("SELECT * FROM tblfiles WHERE rel_id = '".$value->id."' and rel_type = 'office_document'  ")->result();
                                            
                                            if(!empty($file_info)){
                                              foreach ($file_info as $r) {
                                                
                                                 ?>
                                                 <li><i class="fa fa-file" aria-hidden="true"></i> <a download href="<?php echo base_url('uploads/office_document/'.$value->id.'/'.$r->file_name); ?>" ><b><?php echo $r->file_name; ?></b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick="return confirm('Are you sure you want to Remove?');" href="<?php echo admin_url('document/delete_file/'.$r->id.'/'.$parent_id); ?>"><i style="color: red; " class="fa fa-trash" aria-hidden="true"></i></a></li>
                            ...
                            <ul class="tree">
                                <?php
                                 $folder_info  = $this->db->query("SELECT * FROM tbldocuments WHERE id =  '".$parent_id."'  ")->row();
                                     echo '<li style="color:red;"><a href="'.admin_url('document/view/'.$folder_info->parent_id).'"><h4><i class="fa fa-arrow-left" aria-hidden="true"></i> '.cc($folder_info->name).'</h4></a></li>';
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?php echo admin_url('document/create_folder'); ?>" class="proposal-form" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
<div id="file_model" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?php echo admin_url('document/upload_file'); ?>" class="proposal-form" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
$(document).on('click', '.edit_folder', function() { 
    ...
    $.ajax({
            type    : "POST",
            url     : "<?php echo site_url('admin/document/get_editdata'); ?>",
            data    : {'id' : id},
            success : function(response){
                if(response != ''){                   
