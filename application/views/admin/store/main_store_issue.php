                                ...
                                <div class="row">
                                    <div class="col-md-4">  
                                        <div class="form-group">
                                            ...
                                            <select class="form-control selectpicker" required='' data-live-search="true" onchange="getmainstoredata();" id="warehouse_id" name="warehouse_id">
                                                <option value=""></option>
                                                <?php
                                    ...
                                    <div class="col-md-4 issuestorediv">
                                        <div class="form-group" app-field-wrapper="issue_store">
                                            <label for="t_date" class="control-label">Issue Store</label>
                                            <select class="form-control selectpicker" required='' id="issue_store" onchange="getOperators();" name="issue_store" data-live-search="true">
                                                <option value=""></option>
                                                <option value="2" <?php echo (isset($issue_store) && $issue_store == 2) ? "selected=''": ""; ?>>Shop Floor</option>
                                                <option value="3" <?php echo (isset($issue_store) && $issue_store == 3) ? "selected=''": ""; ?>>Finished Goods</option>
                                                <option value="4" <?php echo (isset($issue_store) && $issue_store == 4) ? "selected=''": ""; ?>>Consumable</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-2 operator_div" style="display:none;">
                                        <div class="form-group">
                                            <label for="operator_id" class="control-label">Operators</label>
                                            <select class="form-control selectpicker" data-live-search="true" id="operator_id" name="operator_id">
                                                <option value=""></option>
                                                <?php
                                                if (isset($stafflist) && count($stafflist) > 0) {
                                                    foreach ($stafflist as $staff) {
                                                        $selected = ($operator_id == $staff->staffid) ? "selected": ""; ?>
                                                        ?>
                                                        <option value="<?php echo $staff->staffid; ?>" <?php echo $selected; ?>><?php echo $staff->firstname; ?></option>
                                                        <?php
                                                    }
                                                }
                                                ?>
...
$(document).ready(function() {
    ...
    function getOperators(type){
        var type = $("#issue_store").val();
        if (type == "4"){
            $(".issuestorediv").removeClass("col-md-4");
            $(".issuestorediv").addClass("col-md-2");
            $(".operator_div").show();
        }else{
            $(".issuestorediv").removeClass("col-md-2");
            $(".issuestorediv").addClass("col-md-4");
            $(".operator_div").hide();
        }
    }
