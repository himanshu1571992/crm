...
<div id="wrapper">
    <div class="content accounting-template">
         <div class="row">
            <?php echo form_open_multipart($this->uri->uri_string(), array('id' => 'main_store_issue', 'class' => 'proposal-form', 'onsubmit' => "return confirm('Do you really want to perform cutting action?');")); ?>
            <div class="col-md-12">
                <div class="panel_s">
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="col-md-6">  
                                    <div class="form-group">
                                        <label for="warehouse_id" class="control-label">Product</label>
                                ...
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <label for="size" class="control-label">Total Size <small class="text-danger">( MM )</small></label>
                                ...
                                <div class="col-lg-4 pull-right col-xs-12 col-md-12 total-column">
                                    <div class="panel_s">
                                        <div class="panel-body">
                                            ...
                                            <span class="text-danger">Total Remaining Size</span>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <table class="table credite-note-items-table items table-main-credit-note-edit no-mtop saletable" id="newtable">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th width='15%'>Total size <small>( MM )</small></th>
                                                </tr>
                                            </thead>
                                            <tbody class="product_div">
                                                ...
                                                ?>
                                                            <tr id="products_<?php echo $r; ?>">
                                                                ...
                                                                <td>
                                                                    <select class="form-control selectpicker sub_product_list" onchange="getproductid(<?php echo $r; ?>);" required='' data-live-search="true" id="pro_id<?php echo $r; ?>" name="logdata[<?php echo $r; ?>][pro_id]">
                                                                        <option value=""></option>
                                                ...
                                                <tr id="products_0">
                                                    ...
                                                    <td>
                                                        <select class="form-control selectpicker sub_product_list" onchange="getproductid(0);" required='' data-live-search="true" id="pro_id0" name="logdata[0][pro_id]">
                                                            <option value=""></option>
...
function calculatesize(qty){
    if (qty != ''){
        ...
        if (parseFloat(qty) <= system_qty){
            var ttlsize = parseFloat(qty)*system_size;
...
function calculateproductqty(rowid){
    ...
    var ttlsize = 0;
    if (qty != '' && size != ''){
        ttlsize = parseFloat(qty)*parseFloat(size);
    } 
...
function calculate_remaining_size(){
    ...
    if (sizecount > 0){
        ...
        $(".remainingsize").html(ttlremainingsize);
    }
}
...
function checkForTheCondition(){
    ...
    if (parseFloat(sizecount) != parseFloat(main_size)){
        alert("Total Size not equal to final product size.");
        return false;
    }
    return true;
}
$(document).on('click', '.wastedatabtn', function(){
    ...
    if (wasteqty > 0){
        ...
        $('.product_div').append('<tr id="products_'+newaddmore+'"><td><button style="float: right;" onclick="removeprocomp('+newaddmore+');" class="btn btn-danger" type="button">x</button></td><td><select class="form-control selectpicker sub_product_list" required="" data-live-search="true" onchange="getproductid('+newaddmore+');" id="pro_id'+newaddmore+'" name="logdata['+newaddmore+'][pro_id]"><option value=""></option><?php if(!empty($product_list)) { foreach ($product_list as $key=> $value) { ?> <option value="<?php echo $value->id; ?>"><?php echo $value->sub_name; ?><?php echo " (PRO-ID".$value->id.')'; ?></option> <?php } } ?></select></td><td class="proid'+newaddmore+'"></td><td><input type="number" onkeyup="calculateproductqty('+newaddmore+');" id="qty'+newaddmore+'" name="logdata['+newaddmore+'][qty]" class="form-control" value="'+wasteqty+'" min="0"></td><td><input type="text" id="size'+newaddmore+'" name="logdata['+newaddmore+'][size]" value="'+finalqty+'" onkeyup="calculateproductqty('+newaddmore+');" class="form-control" min="0"></td><td class="ttlsizecount ttlsize'+newaddmore+'" style="font-size: 20px;color: #50f55a;"></td></tr>');
        $('.selectpicker').selectpicker('refresh');
