...
$debitnote_list = $this->db->query("SELECT * from tbldebitnotepayment where  ".$where." order by id desc ")->result();
?>
<h4 class="customer-profile-group-heading">Debit Note</h4>
<div class="row">
    <form method="post" id="salary_form" enctype="multipart/form-data" action="<?php echo admin_url('clients/payment_dn_search'); ?>">
                ...
                <table class="table" id="newtable">
                  ...
                  if(!empty($debitnote_list)){
                    ...
                    foreach($debitnote_list as $row){ 
                      ...
                      <tr>
                        <td><?php echo $z++;?></td>
                        <td><a target="_blank" href="<?php  echo admin_url('debit_note/download_paymentpdf/'.$row->id); ?>"><?php echo $row->number;?></a></td>
