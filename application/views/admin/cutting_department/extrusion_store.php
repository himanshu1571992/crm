...
<?php
if(!empty($this->session->userdata('cutting_extrusion_search'))){
    $search_arr = $this->session->userdata('cutting_extrusion_search');
}    
?>
                                ...
                                <table class="table ui-table">
                                    ...
                                    if(!empty($store_list)){
                                        foreach ($store_list as $key => $row) {       
                                            ...
                                            $transfer_info = $this->db->query("SELECT * from tblextrutiontransfer where  ms_id = '".$row->id."' ")->row();                               
                                            ?>
                                            <tr>
                                                <td><?php echo $z++;?></td>
                                                ...
                                                if($row->transferable == 1){
                                                    if(!empty($transfer_info)){
                                                        echo '<p class="text-warning">Acceptance Pending</p>';
                                                    }else{
                                                        ?>
                                                        <a onclick="return confirm('Are you sure you want to transfer to extrusion department!?');" class="btn btn-info" href="<?php echo admin_url('cutting_department/transfer_stock/'.$row->id);?>">Transfer</a>
                                                        <?php
                                                    }
                                                }else{
                                                    echo '<p class="text-info">On request</p>';
                                                }
