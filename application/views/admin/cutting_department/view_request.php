...
<div id="wrapper">
    <div class="content accounting-template">
		 <div class="row">
            ...
            <form method="post" id="salary_form" enctype="multipart/form-data" action="<?php  echo admin_url('cutting_department'); ?>">
            <div class="col-md-12">
                <div class="panel_s">
                    <div class="panel-body">
             
					<h4 class="no-margin"><?php echo $title; ?> <?php if(check_permission_page(176,'create')){ ?><a href="<?php echo admin_url('cutting_department/add_request'); ?>" class="btn btn-info pull-right" style="margin-top:-6px;">Add Item Request</a><?php } ?></h4>
									...
									if(!empty($request_list)){
										...
										foreach($request_list as $row){	
											...
											<tr>
												...
												<td><?php echo cc(value_by_id('tblproduction_department',$row->approval_department,'name')); ?></td>
												<td><?php echo _d($row->date);?></td>
												<td><?php echo '<p class="'.$cls.'" >'.$approve_status.'</p>'; ?></td>
												<td><?php
												if($row->approve_status == 2){
													 echo '<button type="button" class="btn '.$ccls.' btn-sm confirm" value="'.$row->id.'" data-toggle="modal" data-target="#Confirmation_modal">'.$confirmation_status.'</button>';
												}else{
													echo '--'; 
												}
												?></td>
												<td>
													...
													<a class="btn btn-info" href="<?php  echo admin_url('cutting_department/cutting_request_details/'.$row->id); ?>">View</a>
													<?php 
													if(check_permission_page(176,'delete')){ 
														if($row->approve_status == 1){
													?>
														<a class="btn btn-danger _delete" href="<?php  echo admin_url('cutting_department/delete_request/'.$row->id); ?>">Delete</a>
													<?php
														}
													}
													else
													...
													if($row->confirmation_status == 2 && $row->cutting_process == 1){
														echo '<button type="button" class="btn btn-danger btn-sm tranfer" value="'.$row->id.'" data-toggle="modal" data-target="#trasfer_modal">Cancel & Xfer</button>';
													}
													?>
...
<div id="trasfer_modal" class="modal fade" role="dialog">
...
$(document).on('click', '.tranfer', function() {  
    ...
    $.ajax({
        type    : "POST",
        url     : "<?php echo site_url('admin/cutting_department/get_trnsferdetails'); ?>",
        data    : {'id' : id},
        success : function(response){
            if(response != ''){       

                $('#transfer_data').html(response);  
                $('.selectpicker').selectpicker('refresh');
            }
        }
    })
