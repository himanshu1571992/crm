...
<div id="wrapper">
    <div class="content accounting-template">
        <div class="row">
            <form action="<?php echo admin_url('reducing_department/conversion');?>" class="proposal-form" id="request_form" enctype="multipart/form-data" method="post" accept-charset="utf-8" onsubmit="return confirm('Do you really want to make conversion ?');">
            <div class="col-md-12">
                <div class="panel_s">
                                    ...
                                    <select class="form-control selectpicker" required="" data-live-search="true" id="product_id" name="product_id">
                                        ...
                                        if (isset($product_info) && count($product_info) > 0) {
                                            foreach ($product_info as $value) {
                                                ?>
                                                <option value="<?php echo $value['id'] ?>"><?php echo cc($value['sub_name']); ?></option>
                                                <?php
                                            }
                                        }
                                        ?>
...
<script type="text/javascript">
    $(document).on('click', '#get_component', function() { 
            ...
            if(product_id != '' && quantity != '' && warehouse_id != '' && service_type != ''){
                $.ajax({
                    type    : "POST",
                    url     : "<?php echo base_url(); ?>admin/reducing_department/get_conversion_component_details",
