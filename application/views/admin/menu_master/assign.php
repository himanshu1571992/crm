							...
							<select required="" class="form-control selectpicker" id="staff_id" name="staff_id" data-live-search="true">
								...
								if(!empty($staff_list)){
									foreach($staff_list as $staff){
										?>
										<option value="<?php echo $staff->staffid;?>" <?php if($staffid == $staff->staffid){ echo 'selected'; } ?> ><?php echo $staff->firstname; ?></option>
										<?php
									}
								}
								?>
                                ...
                                <table class="table ui-table">
                                    ...
                                    if(!empty($menu_info)){
                                        ...
                                        foreach($menu_info as $row){  
                                        	...
                                        	$assigned_info  = $this->db->query("SELECT * FROM tblmenuassigned WHERE staff_id = '".$staffid."' and designation_id = 0 and menu_id =  '".$row->id."' ")->row();
