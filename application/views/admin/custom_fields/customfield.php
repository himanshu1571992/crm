                        ...
                        <div class="customers_field_info mbot25 alert alert-info<?php if(isset($custom_field) && $custom_field->fieldto != 'customers' || !isset($custom_field)){echo ' hide';} ?>">
                          ...
                          <div class="select-placeholder form-group">
                            ...
                            <select name="fieldto" id="fieldto" class="selectpicker" data-width="100%" <?php echo $disable; ?> data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
								...
								<option value="raw_material" <?php if(isset($custom_field) && $custom_field->fieldto == 'raw_material'){echo 'selected';} ?>><?php echo _l('custom_field_material'); ?></option>
                            </select>
                          </div>
						  
						  
						   <!-- for expenses  -->
						  <div id="expenses_div" hidden>
						  <div class="form-group">
								<label for="expense_category_id" class="control-label"><?php echo _l('expense_category'); ?></label>
								<select class="form-control selectpicker" multiple data-live-search="true" id="expense_category_id" name="expense_category_id[]">
									...
									if(!empty($expenses_categories)){
										foreach($expenses_categories as $category){
											$selected = '';
											if(!empty($custom_field->expense_category_id)){
													$expense_category_id_arr = explode(',',$custom_field->expense_category_id);
													if(!empty($expense_category_id)){
														foreach($expense_category_id as $row){
															if($row == $category->id){
																$selected = 'selected';
															}
														}
											...
											?>
											<option value="<?php echo $category->id; ?>" <?php echo (isset($custom_field->expense_category_id) && in_array($category->id,$expense_category_id_arr)) ? 'selected' : "" ?> ><?php echo $category->name;?></option>
											<?php
										}
									}
									?>	
						...
						<!-- end  -->	
                           ...
                           <div class="select-placeholder form-group">
                            ...
                            <select name="type" id="type" class="selectpicker"<?php if(isset($custom_field) && total_rows('tblcustomfieldsvalues',array('fieldid'=>$custom_field->id,'fieldto'=>$custom_field->fieldto)) > 0){echo ' disabled';} ?> data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" data-hide-disabled="true">
								...
								<option value="location" <?php if(isset($custom_field) && $custom_field->type == 'location'){echo 'selected';} ?>>Locatoin</option>
...
<!-- Following CSS for expenses -->
<script>
$('#fieldto').on('change', function () {
	 var fieldto = $(this).val();
	 if(fieldto == 'expenses'){
		  $('#expenses_div').show();
		  $("#expense_category_id").prop('required',true);
	 }else{
		 $('#expenses_div').hide();
		  $("#expense_category_id").prop('required',false);
	 }
});	

$( document ).ready(function() {
   var fieldto = $('#fieldto').val();
   if(fieldto == 'expenses'){
		  $('#expenses_div').show();
		  $("#expense_category_id").prop('required',true);
	 }else{
