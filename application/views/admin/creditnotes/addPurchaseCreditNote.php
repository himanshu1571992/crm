				...
				<div class="row">
						...
						<select class="form-control selectpicker" name="vendor_id" required="" id="vendor_id" onchange="get_challan()" data-live-search="true">
                            ...
                            if (isset($vendors_info) && count($vendors_info) > 0) {
                                foreach ($vendors_info as $vendor_value) {
                                    ...
                                    ?>
                                        <option value="<?php echo $vendor_value['id'] ?>" <?php echo (isset($debit_info->vendor_id) && $debit_info->vendor_id == $vendor_value['id']) ? 'selected' : "" ?>><?php echo cc($vendor_value['name']); ?></option>
                                        <?php
                                }
                            }
                            ?>
	                            ...
	                            <select class="form-control selectpicker" id="invoice_id" name="invoice_id" data-live-search="true">
	                                ...
	                                if(!empty($invoice_info)){
	                                	foreach ($invoice_info as $value) {
	                                		?>
	                                		<option value="<?php echo $value->id ;?>" <?php echo (!empty($debit_info->invoice_id) && $debit_info->invoice_id == $value->id) ? 'selected' : '' ; ?>><?php echo 'Inv-'.str_pad($value->id, 4, '0', STR_PAD_LEFT); ?></option>
	                                		<?php
	                                	}
	                                }
	                                ?>
				 ...
				 <div class="col-md-6">
					<div class="panel_s no-shadow">
						<div class="row">
						<?php $value = (isset($debit_info) ? $debit_info->number : get_purchase_creditnote_number()); ?>
...
<script type="text/javascript">
	...
	if(vendor_id > 0){
        ...
        $.ajax({
            type    : "POST",
            url     : "<?php echo site_url('admin/Creditnotes/get_purchase_invoice_list'); ?>",
