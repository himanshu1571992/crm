                                                    ...
                                                    <div class="feed-item">
                                                        ...
                                                        <div class="text <?php echo ($log->status == 2) ? 'line-throught':''; ?>">
                                                            ...
                                                            if ((get_staff_user_id() == $log->staffid) && ($log->status == 1)) {
                                                                ?>
                                                                <a href="<?php echo admin_url('requirement/cut_requirement_conversation/' . $log->id); ?>" onclick="return confirm('Are you sure you want to cut?');"><i class="fa fa-ban" aria-hidden="true"></i></a>
                                                                <?php
                                                            }
                                                            if (is_admin() == 1 && $ttl_reply == 0) {
                                                                ?>
                                                                <a href="<?php echo admin_url('requirement/delete_requirement_conversation/' . $log->id); ?>" val="<?php echo $log->id; ?>" onclick="return confirm('Are you sure you want to Delete?');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                                                <?php
                                                            }
                                                            if ($log->staffid != 0) { ?>
...
<script type="text/javascript">
    $(document).on('click', '.priority', function () {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('admin/requirement/udpate_requirement_priority'); ?>",
...
<script type="text/javascript">
    $(document).on('click', '#load_more', function () {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo site_url('admin/requirement/get_last_requirement_conversion'); ?>",
 ...
 $(document).on("click",".view_reply", function(){
    ...
    $(this).addClass("toggle_reply");
      $.ajax({
          ...
          data    : {'id': last_reply_id, 'logid' : logid, 'section': 'requirement_activity'},
          success : function(response){
            if(response != ''){
