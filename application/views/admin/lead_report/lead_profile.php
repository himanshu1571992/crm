...
<div class="content">
   <div class="row">
      <div class="col-md-2">
         ...
         <?php echo lead_report_tab('profile',$lead_id);?>
      </div>
          ...
          <div class="<?php if(isset($openEdit) && $openEdit == true){echo 'open-edit ';} ?>lead-wrapper" <?php if(isset($lead) && ($lead->junk == 1 || $lead->lost == 1)){ echo 'lead-is-junk-or-lost';} ?>>
   <?php if(isset($lead)){?>
   <div class="btn-group pull-left lead-actions-left">
      <a href="<?php echo base_url();?>/admin/leads/leads/<?php echo $lead->id;?>"  class="mright10 font-medium-xs pull-left <?php if(isset($lead_lock) && $lead_lock == true){echo ' hide';} ?>">
         <?php echo _l('edit'); ?>
      ...
      <ul class="dropdown-menu dropdown-menu-left" id="lead-more-dropdown">
         ...
         <?php } ?>
         <?php } ?>
         <?php if(((is_lead_creator($lead->id) || has_permission('leads','','delete')) && isset($lead_locked) && $lead_locked == false) || is_admin()){ ?>
         <li>
      ...
      ?>
      <?php if(isset($lead_locked) && $lead_locked == false){ ?>
  ...
  {?>
   ...
   <?php if(isset($lead)){
    ...
    $lead_info = $this->db->query("SELECT * FROM tblleads WHERE id = '".$lead->id."' ")->row();
   ...
   <div class="row">
      <div class="lead-view<?php if(!isset($lead)){echo ' hide';} ?>" id="leadViewWrapper">
         <div class="col-md-4 col-xs-12 lead-information-col">
            ...
            <p class="bold font-medium-xs lead-name"><?php echo cc(isset($company) && $company != '' ? $company : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_add_edit_email'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead_info) && $lead_info->email != '' ? '<a href="mailto:'.$lead_info->email.'">' . $lead_info->email.'</a>' : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_website'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead_info) && $lead_info->website != '' ? '<a href="'.maybe_add_http($lead_info->website).'" target="_blank">' . $lead_info->website.'</a>' : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_add_edit_phonenumber'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead_info) && $lead_info->phonenumber != '' ? '<a href="tel:'.$lead_info->phonenumber.'">' . $lead_info->phonenumber.'</a>' : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_company'); ?></p>
            <p class="bold font-medium-xs"><?php echo cc(!empty($company) && $company != '' ? $company : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_address'); ?></p>
            <p class="bold font-medium-xs"><?php echo cc(isset($lead_info) && $lead_info->address != '' ? $lead_info->address : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_city'); ?></p>
            <p class="bold font-medium-xs"><?php echo cc(value_by_id('tblcities',$lead_info->city,'name')); ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_state'); ?></p>
            <p class="bold font-medium-xs"><?php echo cc(value_by_id('tblstates',$lead_info->state,'name')); ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_country'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead) && $lead->country != 0 ? get_country($lead->country)->short_name : 'India') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_zip'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead_info) && $lead_info->zip != '' ? $lead_info->zip : '-') ?></p>
         </div>
         <div class="col-md-4 col-xs-12 lead-information-col">
            <div class="lead-info-heading">
               <h4 class="no-margin font-medium-xs bold">
            ...
            <p class="bold font-medium-xs mbot15"><?php echo cc(isset($lead_data) && $lead_data->source != '' ? $lead_data->source : '-') ?></p>
            <?php if(get_option('disable_language') == 0){ ?>
            <p class="text-muted lead-field-heading"><?php echo _l('remark'); ?></p>
            <p class="bold font-medium-xs mbot15"><?php echo cc(isset($lead_data) && $lead_data->remark != '' ? $lead_data->remark : '-') ?></p>
            <?php } ?>
            ...
            <div>
               <?php 
                  foreach ($contactdata as $key => $value) {
                     $i = $key+1;
                     echo '<p class="text-info bold lead-field-heading">'.$i.') <u>'.cc($value->firstname).'</u></p><br>';
                     echo '<ul>
                              <li> - Email ID : '.$value->email.'</li>
                              <li> - Contact : '.$value->phonenumber.'</li>
                           </ul>';
                  }
               ?>
            </div>
         ...
         <div class="col-md-12">
            ...
            <p class="bold font-medium-xs"><?php echo (isset($lead_data) && isset($lead_data->description) && $lead_data->description != '' ? $lead_data->description : '-') ?></p>
         </div>
      </div>
      <div class="clearfix"></div>
      <div class="lead-edit<?php if(isset($lead)){echo ' hide';} ?>">
    ...
    <?php
       ...
       <div class="col-md-12 leadsdv <?php if(isset($lead_lock) && $lead_lock == false){echo ' hide';} ?>">
...
<!-- <div class="lead-latest-activity lead-view">
   ...
   <?php if(isset($lead_lock) && $lead_locked == false){ ?>
   <div class="lead-edit<?php if(isset($lead)){echo ' hide';} ?>">
      <hr />
