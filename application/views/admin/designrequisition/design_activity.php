                            ...
                            <div class="row">
                                ...
                                <div role="tabpanel" class="tab-pane" id="lead_activity">
                                    <div class="panel_s no-shadow">
                                                    ...
                                                    <div class="feed-item">
                                                        ...
                                                        <div class="text <?php if ($log->status == 2) {
                                                            ...
                                                            if ((get_staff_user_id() == $log->staffid) && ($log->status == 1)) {
                                                                ?>
                                                                <a href="<?php echo admin_url('designrequisition/cut_designremark_conversation/' . $log->id); ?>" onclick="return confirm('Are you sure you want to cut?');"><i class="fa fa-ban" aria-hidden="true"></i></a>
                                                                <?php
                                                            }
                                                            if (is_admin() == 1 && $ttl_reply == 0) {
                                                                ?>
                                                                <a href="<?php echo admin_url('designrequisition/delete_designremark_conversation/' . $log->id); ?>" val="<?php echo $log->id; ?>" onclick="return confirm('Are you sure you want to Delete?');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                                                <?php
                                                            }
                                                            if ($log->staffid != 0) { ?>
                                        ...
                                        <input type="hidden" value="<?php echo $design_activity_id; ?>" id="design_activity_id" name="design_activity_id">
...
<script type="text/javascript">
    $(document).on('click', '.priority', function () {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('admin/designrequisition/udpate_designremark_priority'); ?>",
...
<script type="text/javascript">
    $(document).on('click', '#load_more', function () {
        ...
        var id = $("#last_id").val();
        var design_activity_id = $("#design_activity_id").val();

        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo site_url('admin/designrequisition/get_last_designremark_conversion'); ?>",
            data: {'id': id, 'design_activity_id': design_activity_id},
            success: function (response) {
                if (response != '') {
 ...
 $(document).on("click",".view_reply", function(){
    ...
    $(this).addClass("toggle_reply");
      $.ajax({
          ...
          data    : {'id': last_reply_id, 'logid' : logid, 'section': 'design_activity'},
          success : function(response){
            if(response != ''){
