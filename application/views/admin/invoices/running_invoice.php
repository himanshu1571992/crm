                                    ...
                                    if(!empty($invoice_list)){
                                        foreach ($invoice_list as $key => $value) {
                                            ...
                                            $outputType = '<span class="inline-block label label-' . (empty($color) ? 'default': '') . '" style="color:' . $color . ';border:1px solid ' . $color . '">' . $r_name;
                                                ...
                                                $outputType .= '<ul class="dropdown-menu dropdown-menu-right" aria-labelledby="tableLeadsStatus-' . $value->id . '">';
                                                foreach ($running_data as $running_change) {
                                                    if ($value->running_status != $running_change['id']) {
                                                        $outputType .= '<li>
                                                      <a href="#" onclick="change_running_status(' . $running_change['id'] . ',' . $value->id . '); return false;">
                                                         ' . $running_change['name'] . '
                                                      </a>
                                                   </li>';
                                                    }
...
<script type="text/javascript">
    function change_running_status(status, invoice_id) {
    var data = {};
    data.status = status;
    data.invoice_id = invoice_id;
    $.post('<?php echo base_url(); ?>admin/invoices/update_running_status', data).done(function(response) {
