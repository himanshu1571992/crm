         ...
         <div class="row">
            ...
            <div class="col-md-12">
                <div class="panel_s">
                    ...
                    <h4 class="no-margin">Final Process List <?php if(check_permission_page(113,'create')){ ?><a href="<?php echo admin_url('manufacture_process/final_process'); ?>" class="btn btn-info pull-right" style="margin-top:-6px;">Final Process</a><?php } ?></h4>
                            ...
                            <select class="form-control" id="warehouse_id" name="warehouse_id">
                                ...
                                if(!empty($warehouse_info)){
                                    foreach ($warehouse_info as $key => $value) {
                                        ?>
                                        <option value="<?php echo $value->id;?>" <?php if($s_warehouse_id == $value->id){ echo 'selected';} ?>  ><?php echo $value->name;?></option>
                                        <?php
                                    }
                                }
                                ?>
                        ...
                        <div class="">
                            ...
                            <div class="col-md-12">                                                             
                                <table class="table ui-table">
                                    ...
                                    if(!empty($final_info)){
                                        $i=1;
                                        foreach($final_info as $row){  

                                            ?>
                                            <tr>
                                                <td><?php echo $i++;?></td>
                                                <td><?php echo 'FP-'.str_pad($row->id, 4, "0", STR_PAD_LEFT);?></td>
                                                <td><?php echo $row->reference_no; ?></td>
                                                <td><?php echo value_by_id('tblwarehouse',$row->warehouse_id,'name'); ?></td>
                                                <td><?php echo ($row->service_type == 1) ? 'Rent' : 'Sale'; ?></td>
                                                <td><?php echo value_by_id('tblproducts',$row->product_to_id,'sub_name'); ?></td>
  ...
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Final process details</h4>
      </div>
...
<script type="text/javascript">
    $(document).on('click', '.c_id', function()   
    {
        var id = $(this).val();        
            ...
            $.ajax({
                type    : "POST",
                url     : "<?php echo site_url('admin/manufacture_process/get_final_details'); ?>",
                data    : {'id' : id},
