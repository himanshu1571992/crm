<?php
/**
 * Included in application/views/admin/vendors/vendor.php
 */
?>
<script>
Dropzone.options.vendorAttachmentsUpload = false;
var customer_id = $('input[name="userid"]').val();
$(function() {

    if ($('#vendor-attachments-upload').length > 0) {
        new Dropzone('#vendor-attachments-upload',$.extend({},_dropzone_defaults(),{
            paramName: "file",
    ...
    if (typeof(Dropbox) != 'undefined' && $('#dropbox-chooser').length > 0) {
        document.getElementById("dropbox-chooser").appendChild(Dropbox.createChooseButton({
            success: function(files) {
                $.post(admin_url + 'vendors/add_external_attachment', {
                    files: files,
    ...
    initDataTable('.table-contracts-single-vendor', admin_url + 'contracts/table/' + customer_id, undefined,undefined, 'undefined', [6, 'desc']);
    ...
    <?php } ?>
    _table_api = initDataTable('.table-contacts', admin_url + 'vendors/contacts/' + customer_id, contactsNotSortable, contactsNotSortable);
    if(_table_api) {
          <?php if(is_gdpr() && get_option('gdpr_enable_consent_for_contacts') == '1'){ ?>
    ...
    initDataTable('.table-invoices-single-vendor',
   ...
   initDataTable('.table-credit-notes', admin_url+'credit_notes/table/'+customer_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);
    ...
    initDataTable('.table-estimates-single-vendor',
    ...
    initDataTable('.table-payments-single-vendor',
    ...
    initDataTable('.table-expenses-single-vendor',
    ...
    initDataTable('.table-proposals-vendor-profile',
    ...
    initDataTable('.table-projects-single-vendor', admin_url + 'projects/table/' + customer_id, undefined, undefined, 'undefined', <?php echo do_action('projects_table_default_order',json_encode(array(5,'asc'))); ?>);
    ...
    _validate_form($('.vendor-form'), vRules);
...
function delete_contact_profile_image(contact_id) {
    requestGet('vendors/delete_contact_profile_image/'+contact_id).done(function(){
...
function contact(vendor_id, contact_id) {
    if (typeof(contact_id) == 'undefined') {
        contact_id = '';
    }
    requestGet('vendors/contact/' + vendor_id + '/' + contact_id).done(function(response) {
...
function update_all_proposal_emails_linked_to_contact(contact_id) {
    ...
    $.post(admin_url + 'vendors/update_all_proposal_emails_linked_to_customer/' + contact_id, data).done(function(response) {
...
function do_share_file_contacts(edit_contacts, file_id) {
    ...
    $.post(admin_url + 'vendors/update_file_share_visibility', {
        file_id: file_id,
...
function save_longitude_and_latitude(vendorid) {
    var data = {};
    data.latitude = $('#latitude').val();
    data.longitude = $('#longitude').val();
    $.post(admin_url + 'vendors/save_longitude_and_latitude/'+vendorid, data).done(function(response) {
       if(response == 'success') {
            alert_float('success', "<?php echo _l('updated_successfully', _l('vendor')); ?>");
       }
        setTimeout(function(){
            window.location.reload();
        },1200);
...
// Global function to edit vendor note
function edit_vendor_note(id) {
	
    var description = $("body").find('[data-note-edit-textarea="' + id + '"] textarea').val();
    if (description !== '') {
        $.post(admin_url + 'misc/edit_vendor_note/' + id, {
