<style>.error{border :1px solid red !important;}</style>
   ...
   <h4 class="modal-title">
      <?php if(isset($lead)){
         if(!empty($lead->name)){
		  ...
		  if($lead->leadno!=''){ $leadno= $lead->leadno;}else{$leadno= 'EQ-ID-'.$lead->id;}
         echo '#'.$lead->id . ' - ' . $leadno;
         } else {
  ...
  <div class="row">
   ...
   <div class="top-lead-menu">
      ...
      <ul class="nav-tabs-horizontal nav nav-tabs<?php if(!isset($lead)){echo ' lead-new';} ?>" role="tablist">
         ...
         <?php } ?>
         <li role="presentation">
            <a href="#tab_proposals_leads" onclick="initDataTable('.table-proposals-lead', admin_url + 'proposals/proposal_relations/' + <?php echo $lead->id; ?> + '/proposal','undefined', 'undefined','undefined',[6,'desc']);" aria-controls="tab_proposals_leads" role="tab" data-toggle="tab">
		...
		<!--  <li role="presentation">
            <a href="#lead_approval" aria-controls="lead_approval" role="tab" data-toggle="tab">
            <?php echo _l('lead_approval'); ?>
            </a>
         </li> -->
   ...
   <div class="tab-content">
	  ...
	  <div role="tabpanel" class="tab-pane" id="lead_approval">
         <?php
...
<h4><?php echo $singlegroup['name'];?></h4>
			...
			<div class="col-md-12">
				   <div class="text-right">
					  <button id="lead_approval" class="btn btn-info lead_approval"  value="<?php echo $leadid;?>">Send For Approval</button>
				   </div>
				</div>
			</div>
		...
		<?php
		 ...
		 {
				$leadsql=$this->db->query("SELECT * FROM `tblleads` WHERE `id`='".$leadid."' ")->row_array();
				$leadstaffsql=$this->db->query("SELECT * FROM `tblleadstaffapproval` WHERE `lead_id`='".$leadid."' AND `staff_id`='".get_staff_user_id()."'")->result_array();
				if(count($leadsql)>0)
				{?>
			<div class="panel_s no-shadow leaddv">
			<div class="activity-feed">
			<?php
			   ...
			   foreach($assignlist as $singleassign){ 
			   if(trim($singleassign['firstname'])!=''){?>
			   
				<div style="margin-bottom: 14px;" class="<?php if($leadsql['lead_send']!=1){?>checkbox checkbox-primary<?php }?>">
                    <?php if($leadsql['lead_send']!=1){?><input type="checkbox" name="staffs[]" value="<?php echo $singleassign['staffid'];?>" id="contact_primary<?php echo $singleassign['staffid'];?>" ><?php }?>
                    <label style="width:100%;" for="contact_primary<?php echo $singleassign['staffid'];?>">
                       <span style="font-size:14px;"> <?php echo trim($singleassign['firstname']).' '.trim($singleassign['lastname']).' - '.trim($singleassign['email']); ?>
                   
					<?php 
					if($leadsql['lead_send']==1)
					{
						$leadstaffsql=$this->db->query("SELECT * FROM `tblleadstaffapproval` WHERE `lead_id`='".$leadid."' AND `staff_id`='".$singleassign['staffid']."'")->row_array();
						if($leadstaffsql['approve_status']==0){echo'<span style="margin-left: 6%;color: #fff;border-radius: 9px;padding: 8px;background: #FFC107;">Approval Sent</span></h5>';}else if($leadstaffsql['approve_status']==1){echo'<span style="margin-left: 6%;color: #fff;border-radius: 9px;padding: 8px;background: #4CAF50;">Approval Accepted</span></h5>';}else{echo'<span style="margin-left: 6%;color: #fff;border-radius: 9px;padding: 8px;background: #F44336;">Approval Decline</span></h5></span></label>';}				
					}?>
                </div>
               <?php } } ?>
				...
				<?php if($leadsql['lead_send']!=1){?>
				<div class="col-md-12">
				   <div class="text-right">
					  <button id="lead_approval" class="btn btn-info lead_approval" value="<?php echo $leadid;?>"><?php echo _l('send_for_approval'); ?></button>
				   </div>
				</div>
				<?php }?>
				</div>
			...
			<?php
				...
				else if(count($leadstaffsql)>0)
				{
				$leaddetail=$this->db->query("SELECT * FROM `tblleads` WHERE `id`='".$leadid."'")->row_array();?>
					  ...
					  <div class="col-md-12">
					    ...
					    <div class="text-right">
							...
							<div class="form-group">
								<textarea id="lead_desc" placeholder="Enter Reason"class="form-control lead_description" rows="4" enabled="enabled"></textarea>
							</div>
							<button val="<?php echo $leadid;?>"class="btn btn-success approval" value="1"><?php echo 'Accept'; ?></button>
							<button val="<?php echo $leadid;?>" class="btn btn-info approval" value="2"><?php echo 'Decline'; ?></button>
					    </div>
					  </div>
					</div>
			...
			<?php		
				...
				?>
				<div class="leadSuccess" style="color:green;display:none;    text-align: center;font-size: 19px;color: green;padding: 7px;font-weight: 500;">Lead Send to Staff For Approval.</div>
      </div>
      <div role="tabpanel" class="tab-pane" id="tab_proposals_leads">
         <?php //if(has_permission('proposals','','create')){ ?>
         <a href="<?php echo admin_url('proposals/proposal?rel_type=proposal&rel_id='.$lead->id); ?>" class="btn btn-info mbot25"><?php echo _l('new_proposal'); ?></a>
         <?php //} ?>
         <?php if(total_rows('tblproposals',array('rel_type'=>'proposal','rel_id'=>$lead->id))> 0 && (has_permission('proposals','','create') || has_permission('proposals','','edit'))){ ?>
         <a href="#" class="btn btn-info mbot25" data-toggle="modal" data-target="#sync_data_proposal_data"><?php echo _l('sync_data'); ?></a>
         <?php $this->load->view('admin/proposals/sync_data',array('related'=>$lead,'rel_id'=>$lead->id,'rel_type'=>'proposal')); ?>
         <?php } ?>
         <?php
            $table_data = array(
             _l('proposal') . ' #',
...
$('.lead_approval').click(function(){
   ...
   $('.approval').click(function()
   {
	  ...
	  else
	  {
		  if(lead_description=='')
		  {
			$('.lead_description').addClass('error');  
		  }
		  else
		  {
			$('.lead_description').removeClass('error');    
		  }
	  }
   });
