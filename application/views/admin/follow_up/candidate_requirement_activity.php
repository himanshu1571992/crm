...
<div id="wrapper">
    <div class="content accounting-template">
        <div class="row">
            ...
            <form method="post" id="salary_form" enctype="multipart/form-data" action="<?php echo admin_url('follow_up/candidate_requirement_activity/').$requirement_id; ?>">
                <div class="col-md-12">
                    <div class="panel_s">
                        <div class="panel-body">
                            <h4 class="no-margin"><?php echo $title; ?>  <a href="javascript:void(0);" class="requirement_details btn-sm btn-info" data-id="<?php echo $requirement_id; ?>">View Details</a> <span style="padding-left: 5%;"><button value="1" name="important_search" class="btn btn-info"><i class="fa fa-star" aria-hidden="true"></i>  Search Important</button> <button type="button" id="load_more" class="btn btn-info"><i class="fa fa-refresh fa-spin" aria-hidden="true"></i>  Load last conversation</button> </span></h4>
                            ...
                            <div class="row">
                                ...
                                <div role="tabpanel" class="tab-pane" id="requirement_activity">
                                    <div class="panel_s no-shadow">
                                                        ...
                                                        <div class="feed-item">
                                                            ...
                                                            <div class="text <?php echo ($log->status == 2) ? 'line-throught':''; ?>">
                                                                ...
                                                                if ((get_staff_user_id() == $log->staffid) && ($log->status == 1)) {
                                                                    ?>
                                                                    <a href="<?php echo admin_url('follow_up/cut_requirement_conversation/' . $log->id); ?>" onclick="return confirm('Are you sure you want to cut?');"><i class="fa fa-ban" aria-hidden="true"></i></a>
                                                                    <?php
                                                                }
                                                                if (is_admin() == 1 && $ttl_reply == 0) {
                                                                    ?>
                                                                    <a href="<?php echo admin_url('follow_up/delete_requirement_conversation/' . $log->id); ?>" onclick="return confirm('Are you sure you want to Delete?');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                                                    <?php
                                                                }
                                                                ?>
                                        ...
                                        <input type="hidden" value="<?php echo $requirement_id; ?>" id="requirement_id" name="requirement_id">
...
<?php // echo render_textarea('description','','',array('placeholder'=>_l('enter_activity')),array(),'mtop15');  ?>
                                            <!-- <textarea name="description" id="description" class="form-control mtop15" rows="5" ></textarea> -->
...
<script type="text/javascript">
    $(document).on('click', '.priority', function () {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('admin/follow_up/udpate_requirement_priority'); ?>",
...
<script type="text/javascript">
    $(document).on('click', '#load_more', function () {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo site_url('admin/follow_up/get_last_requirement_conversion'); ?>",
            data: {'id': id, 'requirement_id': requirement_id},
            success: function (response) {
                if (response != '') {
 ...
 $(document).on("click",".view_reply", function(){
    ...
    $(this).addClass("toggle_reply");
      $.ajax({
          ...
          data    : {'id': last_reply_id, 'logid' : logid, 'section': 'candidate_requirement'},
          success : function(response){
            if(response != ''){
