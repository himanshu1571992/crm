...
<tr>
    ...
    <td align="right" width="15%">' . format_money($subtotal,$estimate->symbol) . '</td>
</tr>';
...
foreach ($taxes as $tax) {
	...
	$taxx=$tax['total_tax'];
	 if($tax['taxname']=='GST/SGST'){ $taxxx='GST 9.00% + SGST 9.00%'; }else{$taxxx=$tax['taxname'] . ' (' . _format_number($tax['taxrate']) . '%)';}
    $tbltotal .= '<tr>
	
    <td align="right" width="85%"><strong>' . $taxxx . '</strong></td>
...
if($othercharges[0]['category_name']!='')
		{
			foreach ($othercharges as $othercharge)
			{
				...
				$tbltotal .= '<tr>
								...
								<td align="right" width="15%">' . format_money($othercharge['total_maount'], $estimate->symbol) . '</td>
							</tr>';
			}
		}
		else
...
if($_GET['type']=='rent')
{
	$prototal=(($estimate->rentsubtotal)+array_sum($total_taxe)+array_sum($total_othercharge));
}
if($_GET['type']=='sale')
{
	$prototal=(($estimate->salesubtotal)+array_sum($total_taxe)+array_sum($total_othercharge));
}
if($_GET['type']=='both')
{
	$prototal=(($subtotal)+array_sum($total_taxe)+array_sum($total_othercharge));
}
$tbltotal .= '
<tr style="background-color:#f0f0f0;">
    <td align="right" width="85%"><strong>'._l('estimate_total').'</strong></td>
    <td align="right" width="15%">' . format_money($prototal, $estimate->symbol) . '</td>
</tr>';
