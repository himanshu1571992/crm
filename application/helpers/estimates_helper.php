...
function check_estimate_item($id,$issale)
{
	$CI = & get_instance();
	$proitems=$CI->db->query("SELECT * FROM `tblitems_in` WHERE `rel_id`='".$id."' and `is_sale`='".$issale."' and `rel_type` = 'estimate' ")->result_array();
	$totalproitems=count($proitems);
	return $totalproitems;
}
/**
...
function format_estimate_number($id)
{
    ...
    return $number;*/
    return value_by_id_empty('tblestimates',$id,'number');
}
...
function get_estimates_where_sql_for_staff($staff_id)
{
    ...
    if ($has_permission_view_own) {
       ...
       //$whereUser .= 'sale_agent=' . $staff_id;
		$whereUser .= '(SELECT count(*) as dd FROM `tblestimatestaffapproval` WHERE `staff_id`=' . get_staff_user_id() . ' AND `lead_id`=tblestimates.id)>0 OR (SELECT count(*) as dd FROM `tblestimateassignstaff` WHERE `staff_id`=' . get_staff_user_id() . ' AND `lead_id`=tblestimates.id)>0 OR sale_agent=' . $staff_id;
		//$whereUser .= '(SELECT count(*) as dd FROM `tblestimateassignstaff` WHERE `staff_id`=' . get_staff_user_id() . ' AND `lead_id`=tblestimates.id)>0 OR sale_agent=' . $staff_id;
	}
...
function user_can_view_estimate($id, $staff_id = false)
{
	...
	$estimate= $CI->db->query('SELECT * FROM tblestimates WHERE id='.$id.' AND ((SELECT count(*) as dd FROM `tblestimatestaffapproval` WHERE `staff_id`=' . get_staff_user_id() . ' AND `lead_id`=tblestimates.id)>0 OR (SELECT count(*) as dd FROM `tblestimateassignstaff` WHERE `staff_id`=' . get_staff_user_id() . ' AND `lead_id`=tblestimates.id)>0 OR sale_agent=' . $staff_id.')')->result_array();
...
function user_can_view_chalan($id, $staff_id = false)
{
    $CI = &get_instance();
	...
	$estimate= $CI->db->query("SELECT * FROM `tblchalanmst` WHERE `id`='".$id."' AND ((SELECT count(*) as dd FROM `tblchallanapproval` WHERE `staff_id`='". get_staff_user_id()."' AND `challan_id`=tblchalanmst.id)>0 OR tblchalanmst.`addedfrom`='". get_staff_user_id()."' )")->result_array();
    /*$CI->db->select('id, addedfrom, sale_agent');
    $CI->db->from('tblestimates');
    $CI->db->where('id', $id);
    $estimate = $CI->db->get()->row();
