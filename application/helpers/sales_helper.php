 ...
 * @return boolean
 */
function chalan_details($items,$is_sale)
{
	$CI = & get_instance();
	$i=0;
	$itemHTML='';
	...
	{
		$getcomponentdetails=$CI->db->query("SELECT * FROM `tblproducts` WHERE `id`='".$singleitems['component_id']."'")->row_array();
		$i++;
		$itemHTML .= '<tr' . $trAttributes . ' style="border:1px solid black" >
				<td style="border-right:1px solid black">'.$i.'</td>
				<td style="border-right:1px solid black">'.$getcomponentdetails['name'].'</td>
				<td style="border-right:1px solid black">'.$singleitems['deleverable_qty'].'pcs</td>
...
function transferstock_details($items)
{
	$CI = & get_instance();
	$i=0;
	$itemHTML='';
	foreach($items as $singleitems)
	{
		...
		$itemHTML .= '<tr' . $trAttributes . ' style="border:1px solid black" >
				<td style="border-right:1px solid black">'.$i.'</td>
				<td style="border-right:1px solid black">'.$singleitems['name'].'</td>
				<td style="border-right:1px solid black">'.$singleitems['transfer_qty'].'pcs</td>
				<td style="border-right:1px solid black"></td>
				<td style="border-right:1px solid black"></td>
				<td style="border-right:1px solid black"></td>
			</tr>';
    ...
    function format_customer_info($data, $for, $type, $companyLink = false)
    {
        ...
        if ($for == 'statement') {
            ...
            //$companyName = $data->client->company;
			 //$companyName = get_company_name($clientId);
             $companyName = client_info($clientId)->client_branch_name;
        }
        ...
        if ($companyLink && (!isset($data->deleted_customer_name) || (isset($data->deleted_customer_name) && empty($data->deleted_customer_name)))) {
            $companyName = '<a href="' . admin_url('ClientBranch/branch/' . $clientId) . '" target="_blank"><b>' . $companyName . '</b></a>';
        } elseif ($companyName != '') {
...
if (!function_exists('format_organization_info')) {
    ...
    function format_organization_info()
    {
        ...
        $html = '<b style="color:black" class="company-name-formatted">'.$company_info['company_name'].'</b><br />'.$company_info['address'].'<br /> GST Number: '.$company_info['gst'].'<br /> CIN Number: CIN: 27AAVCS4630C1Z7';
...
function get_items_by_chalan($id)
{
    $CI = &get_instance();
    $CI->db->select();
    //$CI->db->from('tblcreatedchalandetailsmst');
    $CI->db->from('tblchalandetailsmst');
    $CI->db->where('chalan_id', $id);
 ...
 */
function add_new_sales_item_post_inv($item, $rel_id, $rel_type)
{
    $custom_fields = false;
...
function add_new_sales_items_post($item, $rel_id, $rel_type)
{
    $custom_fields = false;
...
if (!function_exists('get_table_items_and_taxe')) {
	function get_pro_detail($itmid,$propid)
	 {
	    $CI = &get_instance();
		$yy= $CI->db->query("SELECT fieldname FROM `tblestimateproductfields` WHERE `proposalid`='".$propid."'")->result_array();
		if(count($yy)>0)
		{
			foreach($yy as $singlefields)
			{
				$tt[]= $singlefields['fieldname'];
			}
			 $fields=implode(',',$tt);
			$rr= $CI->db->query("SELECT ".$fields.",id FROM `tblproducts` WHERE `id`='".$itmid."'")->row_array();
			return	$rr;
		}
		else
		{
			$rr= $CI->db->query("SELECT * FROM `tblproducts` WHERE `id`='".$itmid."'")->row_array();
		}
	}
	function get_table_items_and_taxe($items, $type, $admin_preview = false,$is_sale='')
    {
		//echo $is_sale;exit;
        $cf = count($items) > 0 ? get_items_custom_fields_for_table_htm($items[0]['rel_id'], $type,$is_sale) : [];
        ...
        foreach ($items as $item) {
            $itemHTML        = '';
			$get_pro_details=get_pro_detail($item['pro_id'],$items[0]['rel_id']);
            $trAttributes    = '';
            $tdFirstSortable = '';
            ...
            if (!empty($item['description'])) {
			 ...
			 $itemHTML .= '<br />';
			 if($get_pro_details['sac_code']!=''){ $itemHTML .= '<span style="padding:8px;">SAC Code - '.$get_pro_details['sac_code'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['hsn_code']!='') {$itemHTML .= '<span style="padding:8px;"> HSN Code -'.$get_pro_details['hsn_code'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['working_height']!='') {$itemHTML .= '<span style="padding:8px;"> Working Height -'.$get_pro_details['working_height'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['tower_height']!='') {$itemHTML .= '<span style="padding:8px;"> Tower Height -'.$get_pro_details['tower_height'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['platform_height']!='') {$itemHTML .= '<span style="padding:8px;"> Platform Height -'.$get_pro_details['platform_height'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['dimensions']!=''){ $itemHTML .= '<span style="padding:8px;"> Dimention -'.$get_pro_details['dimensions'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['purchase_price']!=''){ $itemHTML .= '<span style="padding:8px;"> Purchase price -'.$get_pro_details['purchase_price'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['product_weight']!='') {$itemHTML .= '<span style="padding:8px;"> Product Weight -'.$get_pro_details['product_weight'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['product_remarks']!=''){ $itemHTML .= '<span style="padding:8px;"> Product Remarks -'.$get_pro_details['product_remarks'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['product_description']!=''){ $itemHTML .= '<span style="padding:8px;"> Product Description -'.$get_pro_details['product_description'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['damage_rate']!=''){ $itemHTML .= '<span style="padding:8px;"> Damage Rate -'.$get_pro_details['damage_rate'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['lost_rate']!=''){ $itemHTML .= '<span style="padding:8px;"> Lost Rate -'.$get_pro_details['lost_rate'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['repairable_rate']!=''){ $itemHTML .= '<span style="padding:8px;"> Repairable Rate -'.$get_pro_details['repairable_rate'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['lost_rate']!=''){ $itemHTML .= '<span style="padding:8px;"> Lost Rate -'.$get_pro_details['lost_rate'].'</span>'; $itemHTML .= '<br />';}
			 if($get_pro_details['repairable_rate']!=''){ $itemHTML .= '<span > Repairable Rate -'.$get_pro_details['repairable_rate'].'</span>'; $itemHTML .= '<br />';}
...
if (!function_exists('get_table_items_and_taxes')) {
     ...
     * @return array
     */
	 function get_pro_details($itmid,$propid)
	 {
	    $CI = &get_instance();
		$yy= $CI->db->query("SELECT fieldname FROM `tblproposalproductfields` WHERE `proposalid`='".$propid."'")->result_array();
		if(count($yy)>0)
		{
			foreach($yy as $singlefields)
			{
			 ...
			 $fields=implode(',',$tt);
			 //echo "SELECT ".$fields." FROM `tblproducts` WHERE `id`='".$itmid."'";exit;
			$rr= $CI->db->query("SELECT ".$fields.",id FROM `tblproducts` WHERE `id`='".$itmid."'")->row_array();
			return	$rr;
		}
		else
	...
	function get_pro_othercharges($propid,$is_sale)
	 {
		$CI = &get_instance();
		
		//$yy= $CI->db->query("SELECT poc.`total_maount`,oc.`category_name` FROM `tblproposalothercharges` poc LEFT JOIN `tblotherchargemaster` oc on poc.category_name=oc.id WHERE poc.`proposalid`='".$propid."' AND poc.`is_sale`='".$is_sale."' and poc.category_name > 0 ORDER BY poc.id ASC")->result_array();
        $yy= $CI->db->query("SELECT poc.`total_maount`,oc.`category_name` FROM `tblproposalothercharges` poc LEFT JOIN `tblotherchargemaster` oc on poc.category_name=oc.id WHERE poc.`proposalid`='".$propid."' AND poc.`is_sale`='".$is_sale."' and poc.category_name > 0 ORDER BY poc.id ASC")->result_array();
		return	$yy;
	}
	function get_pro_estimate_othercharges($propid,$is_sale)
	{
		$CI = &get_instance();
		if($is_sale==2)
		{
			$yy= $CI->db->query("SELECT poc.`total_maount`,oc.`category_name` FROM `tblestimateothercharges` poc LEFT JOIN `tblotherchargemaster` oc on poc.category_name=oc.id WHERE poc.`proposalid`='".$propid."' and poc.category_name > 0 ORDER BY poc.id ASC")->result_array();
		}
		else
		{
			$yy= $CI->db->query("SELECT poc.`total_maount`,oc.`category_name` FROM `tblestimateothercharges` poc LEFT JOIN `tblotherchargemaster` oc on poc.category_name=oc.id WHERE poc.`proposalid`='".$propid."' AND poc.`is_sale`='".$is_sale."' and poc.category_name > 0 ORDER BY poc.id ASC")->result_array();
		}
		return	$yy;
	}
	function get_pro_invoice_othercharges($propid,$is_sale)
	{
		$CI = &get_instance();
		if($is_sale==2)
		{
			$yy= $CI->db->query("SELECT poc.`total_maount`,oc.`category_name` FROM `tblinvoiceothercharges` poc LEFT JOIN `tblotherchargemaster` oc on poc.category_name=oc.id WHERE poc.`proposalid`='".$propid."' and poc.category_name > 0 ORDER BY poc.id ASC")->result_array();
		}
		else
		{
			$yy= $CI->db->query("SELECT poc.`total_maount`,oc.`category_name` FROM `tblinvoiceothercharges` poc LEFT JOIN `tblotherchargemaster` oc on poc.category_name=oc.id WHERE poc.`proposalid`='".$propid."' AND poc.`is_sale`='".$is_sale."' and poc.category_name > 0 ORDER BY poc.id ASC")->result_array();
		}
		return	$yy;
	}
	
    function get_table_items_and_taxes($items, $type, $admin_preview = false,$saletype)
    {
		$CI = &get_instance();
        $cf = count($items) > 0 ? get_items_custom_fields_for_table_html($items[0]['rel_id'], $type) : [];
		$yy= $CI->db->query("SELECT fieldname FROM `tblproposalproductfields` WHERE `proposalid`='".$items[0]['rel_id']."'")->result_array();
        ...
        foreach ($items as $item) {
			...
			if($saletype=='sale'){if($item['is_sale']==1){
			$get_pro_details=get_pro_details($item['pro_id'],$items[0]['rel_id']);
            $itemHTML        = '';
            $trAttributes    = '';
            $tdFirstSortable = '';
            ...
            $itemHTML .= '<tr' . $trAttributes . '>';
            $itemHTML .= '<td' . $tdFirstSortable . ' align="center">' . $i . '</td>';
			if($get_pro_details['photo']==''){$img=base_url('assets/images/no_image_available.jpeg');}else{$img=base_url('uploads/product')."/".$get_pro_details['id']."/".$get_pro_details['photo'];}
			if(in_array('photo',$fieldname)){ $itemHTML .= '<td><img  src="'.$img.'" >'; $itemHTML .= '</td>';}
            $itemHTML .= '<td class="description" align="left;">';
            if (!empty($item['description'])) {
                $itemHTML .= '<span style="font-size:' . (isset($font_size) ? $font_size + 4 : '') . 'px;"><strong>' . $item['description'] . '</strong></span>';
            ...
            if (count($item_taxes) > 0) {
                ...
                foreach ($item_taxes as $tax) {
                    ...
                    if (get_option('show_tax_per_item') == 1) {
                        ...
                        $hook_data = do_action('item_tax_table_row', $hook_data);
                        $item_tax  = $hook_data['final_tax_html'];
						if($tax['taxname']=='GST/SGST|18.00'){$item_tax='GST 9.00% <br/> SGST 9.00%';}
                        $itemHTML .= $item_tax;
                    }
                }
            } else {
	...
	}if($saletype=='rent'){if($item['is_sale']==0){
			$totalmonths=$item['months']+($item['days']/30).'<br>';
			$get_pro_details=get_pro_details($item['pro_id'],$items[0]['rel_id']);
			//print_r($get_pro_details);exit;
            $itemHTML        = '';
            $trAttributes    = '';
            $tdFirstSortable = '';
			...
			if($get_pro_details['photo']==''){$img=base_url('assets/images/no_image_available.jpeg');}else{$img=base_url('uploads/product')."/".$get_pro_details['id']."/".$get_pro_details['photo'];}
			if(in_array('photo',$fieldname)){ $itemHTML .= '<td><img  src="'.$img.'" >'; $itemHTML .= '</td>';}
            $itemHTML .= '<td class="description" align="left;">s';
            if (!empty($item['description'])) {
                $itemHTML .= '<span style="font-size:' . (isset($font_size) ? $font_size + 4 : '') . 'px;"><strong>' . $item['description'] . '</strong></span>';
			...
			$itemHTML .= '<br />';
					 ...
					 //if($get_pro_details['name']!='') {$itemHTML .= '<span>'.$get_pro_details['name'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['sac_code']!=''){ $itemHTML .= '<span>SAC Code - '.$get_pro_details['sac_code'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['hsn_code']!='') {$itemHTML .= '<span> HSN Code -'.$get_pro_details['hsn_code'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['working_height']!='') {$itemHTML .= '<span> Working Height -'.$get_pro_details['working_height'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['tower_height']!='') {$itemHTML .= '<span> Tower Height -'.$get_pro_details['tower_height'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['platform_height']!='') {$itemHTML .= '<span> Platform Height -'.$get_pro_details['platform_height'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['dimensions']!=''){ $itemHTML .= '<span> Dimention -'.$get_pro_details['dimensions'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['purchase_price']!=''){ $itemHTML .= '<span> Purchase price -'.$get_pro_details['purchase_price'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['product_weight']!='') {$itemHTML .= '<span> Product Weight -'.$get_pro_details['product_weight'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['product_remarks']!=''){ $itemHTML .= '<span> Product Remarks -'.$get_pro_details['product_remarks'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['product_description']!=''){ $itemHTML .= '<span> Product Description -'.$get_pro_details['product_description'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['damage_rate']!=''){ $itemHTML .= '<span> Damage Rate -'.$get_pro_details['damage_rate'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['lost_rate']!=''){ $itemHTML .= '<span> Lost Rate -'.$get_pro_details['lost_rate'].'</span>'; $itemHTML .= '<br />';}
					 if($get_pro_details['repairable_rate']!=''){ $itemHTML .= '<span> Repairable Rate -'.$get_pro_details['repairable_rate'].'</span>'; $itemHTML .= '<br />';}
					// if($get_pro_details['photo']!=''){ $itemHTML .= '<img src="'.base_url('uploads/product')."/".$get_pro_details['id']."/".$get_pro_details['photo'].'">'; $itemHTML .= '<br />';}
            ...
            $itemHTML .= '</td>';
			$itemHTML .= '<td align="right">' . $totalmonths.'</td>';
            $itemHTML .= '<td align="right">' . _format_number($item['rate']) . '</td>';
            if (get_option('show_tax_per_item') == 1) {
                $itemHTML .= '<td align="right">';
            }
            ...
            if (count($item_taxes) > 0) {
                ...
                foreach ($item_taxes as $tax) {
                    ...
                    if ($tax_not_calc == true) {
                        $result['taxes'][$tax['taxname']]          = [];
                        $result['taxes'][$tax['taxname']]['total'] = [];
                        array_push($result['taxes'][$tax['taxname']]['total'], (($item['qty'] * $item['rate']*$totalmonths) / 100 * $tax['taxrate']));
                        $result['taxes'][$tax['taxname']]['tax_name'] = $tax['taxname'];
                        $result['taxes'][$tax['taxname']]['taxrate']  = $tax['taxrate'];
                    } else {
                        array_push($result['taxes'][$tax['taxname']]['total'], (($item['qty'] * $item['rate']*$totalmonths) / 100 * $tax['taxrate']));
            ...
            $hook_data = do_action('final_item_amount', [
                'amount'     => ($item['qty'] * $item['rate'] *$totalmonths),
                'item_taxes' => $item_taxes,
                'item'       => $item,
            ]);
