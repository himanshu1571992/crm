    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($connection_id != '') {
            if ($connection_type == 'proposal') {
                $where_clients = 'CASE
                WHEN tblclientbranch.userid NOT IN(SELECT rel_id FROM tblproposals WHERE id=' . $connection_id . ' AND rel_type="customer") THEN tblclientbranch.active=1
                ELSE 1=1
                END';
            } elseif ($connection_type == 'task') {
                $where_clients = 'CASE
                WHEN tblclientbranch.userid NOT IN(SELECT rel_id FROM tblstafftasks WHERE id=' . $connection_id . ' AND rel_type="customer") THEN tblclientbranch.active=1
                ELSE 1=1
                END';
            }
        }
        ...
        if ($q) {
            ...
            $data = $CI->client_model->get_ajax($rel_id, $where_clients);
		}
		else
		{
			$CI->load->model('clients_model');
			$data=$CI->db->query("SELECT * FROM `tblclientbranch` WHERE `userid`='".$rel_id."'")->row_array();
		}
...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        if (is_array($relation)) {
            $id   = $relation['userid'];
            $name = $relation['client_branch_name'];
        } else {
            $id   = $relation->userid;
