 ...
 * @return void
 */
function check_proposal_item($id,$issale,$rel_type)
{
	$CI = & get_instance();
	$proitems=$CI->db->query("SELECT * FROM `tblitems_in` WHERE `rel_id`='".$id."' and `is_sale`='".$issale."' AND `rel_type`='".$rel_type."' order by id asc")->result_array();
...
function format_proposal_number($id)
{
    ...
    return value_by_id_empty('tblproposals',$id,'number');
}

function format_stock_number($id)
{
    return 'WS-ID-' . str_pad($id, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
}
/**
...
function get_proposals_sql_where_staff($staff_id)
{
    ...
    if ($has_permission_view_own) {
       ...
       // $whereUser .= '(SELECT count(*) as dd FROM `tblproposalstaffapproval` WHERE `staff_id`=' . get_staff_user_id() . ' AND `lead_id`=tblproposals.id)>0 OR (SELECT count(*) as dd FROM `tblproposalassignstaff` WHERE `staff_id`=' . get_staff_user_id() . ' AND `lead_id`=tblproposals.id)>0 OR assigned=' . $staff_id;
    }
