...
function get_items_custom_fields_for_table_htm($rel_id, $rel_type,$is_sale) {
    $whereSQL = 'id IN (SELECT fieldid FROM tblcustomfieldsvalues WHERE value != "" AND value IS NOT NULL AND fieldto="items" AND relid IN (SELECT id FROM tblitems_in WHERE rel_type="' . $rel_type . '" AND rel_id="' . $rel_id . '" AND is_sale="1") GROUP BY id HAVING COUNT(id) > 0)';
...
function is_custom_fields_for_customers_portal() {
    if (is_client_logged_in() || (!is_staff_logged_in() && !is_client_logged_in()) || defined('EMAIL_TEMPLATE_SEND') || defined('CLIENTS_AREA') || defined('GDPR_EXPORT') || DEFINED('CRON')) {
        return true;
    }
