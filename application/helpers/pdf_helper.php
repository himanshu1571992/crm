...
function pdf_logo_url()
{
    ...
    return 'http://schachengineers.com/schacrm/uploads/company/logo.png';
}
/**
...
function chalan_pdf($estimate, $tag = '')
{
    $GLOBALS['estimate_pdf'] = $estimate;
    $CI                      = & get_instance();
    load_pdf_language($estimate->clientid);
    ...
    if (file_exists(APPPATH . 'views/themes/' . active_clients_theme() . '/views/my_estimatepdf.php')) {
        ...
        include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/chalanpdf.php');
    }
...
function stock_transfer_pdf($estimate, $tag = '')
{
    $GLOBALS['estimate_pdf'] = $estimate;
    $CI                      = & get_instance();
    load_pdf_language($estimate->clientid);
    ...
    if (file_exists(APPPATH . 'views/themes/' . active_clients_theme() . '/views/my_estimatepdf.php')) {
        ...
        include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/stocktransferpdf.php');
    }
 ...
 * @return object
 */
function proposal_pdf($proposal, $tag = '',$type)
{
...
function stock_pdf($warehousestock, $tag = '',$type)
{
    $GLOBALS['proposal_pdf'] = $warehousestock;
    $CI                      = & get_instance();
    ...
    if (!file_exists(APPPATH . 'libraries/Proposal_pdf.php')) {
        $pdf = new Pdf($formatArray['orientation'], 'mm', $formatArray['format'], true, 'UTF-8', false, false, 'warehousestock');
    } else {
        include_once(APPPATH . 'libraries/Proposal_pdf.php');
        $pdf = new Proposal_pdf($formatArray['orientation'], 'mm', $formatArray['format'], true, 'UTF-8', false, false);
    }
    ...
    $proposal_url = site_url('proposal/' . $warehousestock->id . '/' . $warehousestock->hash);
    $number       = format_proposal_number($warehousestock->id);
    ...
    $CI->load->model('currencies_model');
    $total = '';
    if ($warehousestock->total != 0) {
        $total = format_money($warehousestock->total, $CI->currencies_model->get($warehousestock->currency)->symbol);
        $total = _l('proposal_total') . ': ' . $total;
    }
    # Don't remove these lines - important for the PDF layout
    // Add <br /> tag and wrap over div element every image to prevent overlaping over text
    $warehousestock->content = preg_replace('/(<img[^>]+>(?:<\/img>)?)/i', '<div>$1</div>', $warehousestock->content);
    // Add cellpadding to all tables inside the html
    $warehousestock->content = preg_replace('/(<table\b[^><]*)>/i', '$1 cellpadding="4">', $warehousestock->content);
    // Remove white spaces cased by the html editor ex. <td>  item</td>
    $warehousestock->content = preg_replace('/[\t\n\r\0\x0B]/', '', $warehousestock->content);
    $warehousestock->content = preg_replace('/([\s])\1+/', ' ', $warehousestock->content);

    // Tcpdf does not support float css we need to adjust this here
    $warehousestock->content = str_replace('float: right', 'text-align: right', $warehousestock->content);
    $warehousestock->content = str_replace('float: left', 'text-align: left', $warehousestock->content);
    // Image center
    $warehousestock->content = str_replace('margin-left: auto; margin-right: auto;', 'text-align:center;', $warehousestock->content);

    $warehousestock = do_action('proposal_html_pdf_data', $warehousestock);

    _bulk_pdf_export_maybe_tag($tag, $pdf);
    if (file_exists(APPPATH . 'views/themes/' . active_clients_theme() . '/views/my_proposalpdf.php')) {
        include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/my_proposalpdf.php');
    } else {
        include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/stockpdf.php');
    }
...
function stock_availability_pdf($pdf_data, $tag = '')
{
   //$GLOBALS['pdf_data'] = $pdf_data;
    $CI                      = & get_instance();
    ...
    _bulk_pdf_export_maybe_tag($tag, $pdf);
     include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/stockavailabilitypdf.php');
