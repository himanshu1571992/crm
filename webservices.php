...
if(((isset($username)) && ($username != '')) && ((isset($password)) && ($password != '')) && ((isset($token_id)) && ($token_id != '')) && ((isset($type)) && ($type == 'user_login'))) 
{
	...
	if($chk_login_numrows > 0)
	{
			...
			if($hash_password == $chk_login_row['password']){
...
elseif (((isset($staffid)) && ($staffid != '')) && ((isset($type)) && ($type == 'logout'))) {
	# code...
	$return_status =array();
	$chk_logout_sql = "select * from `tblstaff` where (`staffid`='".$staffid."' ) and active = 1 ";	
...
*/	
}elseif(((isset($username)) && ($username != '')) && ((isset($token_id)) && ($token_id != '')) && ((isset($type)) && ($type == 'fakeLogin'))) 
{
